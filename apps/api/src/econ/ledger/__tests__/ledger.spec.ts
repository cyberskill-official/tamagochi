export type LedgerSpecDecision = {
  allowed: boolean;
  reason: string;
  auditEvent: string;
  tenantId: string;
...
export class LedgerSpec {
  readonly kind = 'economy';

  evaluate(input: { tenantId?: string; audience?: '13+' | 'under-13'; enabled?: boolean; unsafe?: boolean }): LedgerSpecDecision {
...
export const ledgerSpec = new LedgerSpec();
