export type BattlePassSpecDecision = {
  allowed: boolean;
  reason: string;
  auditEvent: string;
  tenantId: string;
...
export class BattlePassSpec {
  readonly kind = 'media';

  evaluate(input: { tenantId?: string; audience?: '13+' | 'under-13'; enabled?: boolean; unsafe?: boolean }): BattlePassSpecDecision {
...
export const battlePassSpec = new BattlePassSpec();
