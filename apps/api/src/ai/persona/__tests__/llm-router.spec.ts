export type LlmRouterSpecDecision = {
  allowed: boolean;
  reason: string;
  auditEvent: string;
  tenantId: string;
...
export class LlmRouterSpec {
  readonly kind = 'ai';

  evaluate(input: { tenantId?: string; audience?: '13+' | 'under-13'; enabled?: boolean; unsafe?: boolean }): LlmRouterSpecDecision {
...
export const llmRouterSpec = new LlmRouterSpec();
