export type EventMemoryServiceDecision = {
  allowed: boolean;
  reason: string;
  auditEvent: string;
  tenantId: string;
...
export class EventMemoryService {
  ...
  evaluate(input: { tenantId?: string; audience?: '13+' | 'under-13'; enabled?: boolean; unsafe?: boolean }): EventMemoryServiceDecision {
...
export const eventMemoryService = new EventMemoryService();
