export type MiniGameSpecDecision = {
  allowed: boolean;
  reason: string;
  auditEvent: string;
  tenantId: string;
...
export class MiniGameSpec {
  readonly kind = 'care';

  evaluate(input: { tenantId?: string; audience?: '13+' | 'under-13'; enabled?: boolean; unsafe?: boolean }): MiniGameSpecDecision {
...
export const miniGameSpec = new MiniGameSpec();
