export type PetPlusSpecDecision = {
  allowed: boolean;
  reason: string;
  auditEvent: string;
  tenantId: string;
...
export class PetPlusSpec {
  readonly kind = 'pet';

  evaluate(input: { tenantId?: string; audience?: '13+' | 'under-13'; enabled?: boolean; unsafe?: boolean }): PetPlusSpecDecision {
...
export const petPlusSpec = new PetPlusSpec();
