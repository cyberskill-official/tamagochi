export type SleepHourSpecDecision = {
  allowed: boolean;
  reason: string;
  auditEvent: string;
  tenantId: string;
...
export class SleepHourSpec {
  readonly kind = 'media';

  evaluate(input: { tenantId?: string; audience?: '13+' | 'under-13'; enabled?: boolean; unsafe?: boolean }): SleepHourSpecDecision {
...
export const sleepHourSpec = new SleepHourSpec();
