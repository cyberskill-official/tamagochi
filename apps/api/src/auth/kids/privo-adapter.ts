export type PrivoAdapterDecision = {
  allowed: boolean;
  reason: string;
  auditEvent: string;
  tenantId: string;
...
export class PrivoAdapter {
  readonly kind = 'compliance';

  evaluate(input: { tenantId?: string; audience?: '13+' | 'under-13'; enabled?: boolean; unsafe?: boolean }): PrivoAdapterDecision {
...
export const privoAdapter = new PrivoAdapter();
