export type InventorySpecDecision = {
  allowed: boolean;
  reason: string;
  auditEvent: string;
  tenantId: string;
...
export class InventorySpec {
  ...
  evaluate(input: { tenantId?: string; audience?: '13+' | 'under-13'; enabled?: boolean; unsafe?: boolean }): InventorySpecDecision {
...
export const inventorySpec = new InventorySpec();
