...
created: 2026-05-17
shipped: 2026-05-17
related_frs: [FR-SUB-001, FR-AUTH-003, FR-LEGAL-001, FR-LEGAL-003, FR-AI-002, FR-ADS-002, FR-VIRAL-005, FR-INFRA-003, FR-ECON-002, FR-OBS-001]
depends_on: [FR-SUB-001, FR-AUTH-003]
blocks: []
effort_hours: 10
new_files:
...
allowed_tools:
  ...
  - "FR-AUTH-003 parental consent flow"
  - "FR-LEGAL-001 DPO surfaces"
disallowed_tools:
...
- Content filter override (parent can disable AI dialogue per FR-AI-001 even for 13+ children).
...
- Enter child's display name (filtered per FR-PET-001 §1.5).
- Enter child's birth year (used to set `audience_age_gate` correctly).
- Triggers FR-AUTH-003 parental consent via Safe Harbor vendor — even though the parent IS the parent, COPPA-2025 requires VPC per child.
...
§1.14  **DSR cascade.** Per FR-LEGAL-001 §3.6 — deleting parent's account triggers child profile review (parent confirms child's data also deletes). Child data deletion follows kids-SKU 30-day retention rules.
...
§1.17  **Locale parental copy.** All parental UI strings reviewed by DPO + localised per FR-I18N-001.
...
§1.20  **Analytics.** `sub.family.subscribed`, `sub.family.child_added`, `sub.family.spend_approved`, `sub.family.spend_rejected`, `sub.family.screen_cap_hit`, `sub.family.child_paused` per FR-OBS-001.
...
**Why parental dashboard mandatory.** COPPA-2025 §1.5 + FR-AUTH-003. Without it, kids SKU has no parental oversight surface beyond Apple/Google level.
...
async addChild(u: AuthedUser, displayName: string, birthYear: number) {
  ...
  // Trigger FR-AUTH-003 parental consent flow.
...
**AC2.** Adding a child triggers FR-AUTH-003 consent flow. Verified.
...
```typescript
describe('FR-SUB-002 — Family tier', () => {
...
**External:** Apple Family Sharing API; Google Family Library API; FR-AUTH-003 Safe Harbor vendor.
**Internal:** FR-SUB-001 (Pet+ baseline), FR-AUTH-003 (parental consent), FR-AI-001/002 (content filter override), FR-LEGAL-001 (DSR + DPO).
**Blocks:** none.
...
**Cross-reference:** FR-SUB-001 baseline; FR-AUTH-003 consent; FR-AI-002 filter; FR-VIRAL-005 push.
