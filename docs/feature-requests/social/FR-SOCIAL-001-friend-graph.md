...
created: 2026-05-17
shipped: 2026-05-17
related_frs: [FR-AUTH-001, FR-AUTH-003, FR-INFRA-002, FR-INFRA-003, FR-LEGAL-001, FR-SOCIAL-002, FR-SOCIAL-003, FR-SOCIAL-004, FR-OBS-001, FR-SUB-002]
depends_on: [FR-AUTH-003, FR-INFRA-002]
blocks: [FR-SOCIAL-002, FR-SOCIAL-003, FR-SOCIAL-004]
effort_hours: 10
new_files:
...
allowed_tools:
  - "Crockford base32 invite codes with checksum (FR-AUTH-003 pattern)"
  - "Colyseus RedisPresence for online status"
  - "Postgres `friends` join table"
disallowed_tools:
  - "Name search on kids SKU (FR-LEGAL-001 + FR-AUTH-003 invite-code-only)"
...
risk_if_skipped: "Plan §PART 3 social hook + plan §PART 7 viral loops. FR-SOCIAL-002 PetPair / FR-SOCIAL-003 trade / FR-SOCIAL-004 wedding ALL require a friend graph. Without it, the social P2 slice collapses."
...
§1.2  **Invite codes.** Per FR-AUTH-003 §1.2, invite codes are 8-char Crockford base32 with checksum. Each player generates a personal invite code on demand (regenerable). Active code is one-per-player; regenerating invalidates the prior.
...
§1.6  **Block.** `POST /v1/friends/:userId/block` adds to blocklist. Blocked users cannot send invite codes that resolve to your account; existing friendship is removed; any pending breeding (FR-PET-007 will allow cross-player in P3+) is cancelled.

§1.7  **No name search on kids SKU.** Per FR-LEGAL-001 §1.5 + FR-AUTH-003, kids SKU MUST only allow invite-code addition. No "find friends by username." Standard SKU may surface "people you may know" only via co-parent introduction (P3+).
...
§1.9  **Presence via Colyseus.** Online status MUST come from Colyseus RedisPresence (FR-INFRA-002 §1.4). Updated in real-time as friends connect/disconnect from their PetRoom.
...
§1.13  **Cross-SKU scoping.** A 13+ player CANNOT friend an under-13 player directly. Under-13 friends only other under-13 players via invite code (kids SKU). FR-AUTH-001 §1.9 cross-SKU isolation extends here.
...
§1.15  **Audit + analytics.** `social.friend.code_generated`, `social.friend.invite_redeemed`, `social.friend.accepted`, `social.friend.rejected`, `social.friend.removed`, `social.friend.blocked`, `social.friend.limit_reached` per FR-OBS-001.
...
§1.18  **DSR.** Per FR-LEGAL-001, friend list deletion is part of account-deletion cascade.
...
**Why invite code over username search on kids.** FR-AUTH-003 + FR-LEGAL-001 §1.5 — under-13 + COPPA. Invite code requires explicit out-of-band sharing.
...
```typescript
// apps/api/src/social/friends/__tests__/friends.spec.ts
describe('FR-SOCIAL-001 — friends', () => {
...
**External:** Colyseus RedisPresence (FR-INFRA-002).
**Internal:** FR-AUTH-003 (kid SKU separation), FR-LEGAL-001 (DSR + alias), FR-SUB-002 (Pet+ limit raise).
**Blocks:** FR-SOCIAL-002/003/004.
...
| 6 | Friend limit Pet+ entitlement bug | Stub returns wrong | FR-SUB-001 wired later | Conservative cap until then |
...
**Cross-reference:** FR-SOCIAL-002 PetPair / FR-SOCIAL-003 trade / FR-SOCIAL-004 wedding all consume this graph.
