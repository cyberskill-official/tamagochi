...
created: 2026-05-17
shipped: 2026-05-17
related_frs: [FR-SOCIAL-001, FR-PET-001, FR-CARE-001, FR-CARE-002, FR-CARE-003, FR-INFRA-002, FR-VIRAL-005, FR-OBS-001, FR-LEGAL-001, FR-AUTH-003]
depends_on: [FR-SOCIAL-001, FR-CARE-001, FR-CARE-002, FR-CARE-003]
blocks: [FR-SOCIAL-003, FR-SOCIAL-004]
effort_hours: 14
new_files:
...
allowed_tools:
  - "FR-PET-001 pet_co_parents table (provisioned at P1)"
  - "Colyseus PetRoom maxClients=2"
  - "FR-VIRAL-005 push notifications"
disallowed_tools:
  - "Co-parent for under-13 SKU without parental dashboard approval (FR-AUTH-003)"
...
§1.2  **Eligibility.** Initial owner must be at `stage ∈ {teen, adult}` per FR-PET-002 §1.6. Both players must be **friends** per FR-SOCIAL-001. Both must be on the standard SKU (no kid co-parenting at P1/P2; parental-dashboard-controlled in P3 via FR-SUB-002).
...
§1.10  **Receipt push.** When a co-parent performs a care action, the OTHER party receives a push notification (FR-VIRAL-005 sleep-hour respected): "Linh fed Mochi while you were asleep ❤️" — copy locked in `apps/cocos/assets/i18n/<locale>/petpair.json`.
...
§1.13  **Source attribution.** Care actions performed by co-parent tagged `source='co_parent_remote'` per FR-CARE-001 §1.17. Audit + analytics distinguishable.
...
§1.17  **Cross-SKU + cross-tenant locked.** Cross-SKU forbidden per §1.2. Cross-tenant forbidden per FR-INFRA-003.

§1.18  **Audit + analytics.** `social.coparent.invite_sent`, `social.coparent.accepted`, `social.coparent.declined`, `social.coparent.action { source }`, `social.coparent.breakup_warned`, `social.coparent.ended { trigger: 'owner' | 'coparent' | 'breakup' }` per FR-OBS-001.

§1.19  **TikTok share moments.** Per FR-VIRAL-001, breakup-warning moment is a documented share-worthy beat (sad/dramatic). Opt-in share button surfaces.
...
**Why both standard SKU only at P2.** FR-AUTH-003 + COPPA — kids social interactions go through parental dashboard. P3 FR-SUB-002 wires the parental gate.
...
```typescript
// apps/api/src/social/coparent/__tests__/breakup-watch.spec.ts
describe('FR-SOCIAL-002 — breakup watch', () => {
...
**External:** Push notification system (FR-VIRAL-005).
**Internal:** FR-SOCIAL-001 (friend graph), FR-CARE-001/002/003 (care actions with source attribution), FR-INFRA-002 (Colyseus PetRoom maxClients=2), FR-VIRAL-001 (break-up share moment).
**Blocks:** FR-SOCIAL-003 (trade needs co-parent presence model), FR-SOCIAL-004 (wedding builds on co-parent).
...
|---|---|---|---|---|
| 1 | Concurrent care from owner + co-parent | Per-pet mutex | None | Already covered by FR-PET-003 |
| 2 | Receipt push during sleep hours | FR-VIRAL-005 | Suppressed | OK |
...
**Cross-reference:** FR-SOCIAL-003 trade window uses presence model. FR-SOCIAL-004 wedding builds on co-parent intimacy.
