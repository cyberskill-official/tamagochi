...
created: 2026-05-17
shipped: 2026-05-17
related_frs: [FR-SOCIAL-001, FR-SOCIAL-002, FR-PET-002, FR-CARE-003, FR-INFRA-002, FR-ART-001, FR-VIRAL-001, FR-OBS-001]
depends_on: [FR-SOCIAL-002, FR-VIRAL-001]
blocks: []
effort_hours: 10
new_files:
...
allowed_tools:
  ...
  - "FR-ART-001 wedding_pose contract animation"
  - "FR-VIRAL-001 share-clip export"
disallowed_tools:
  - "Real-money wedding fee (FR-LEGAL-002)"
...
§1.2  **Eligibility.** Both pets `stage='adult'`; both players friends (FR-SOCIAL-001); both standard SKU; neither pet currently in another wedding; cross-tenant blocked.
...
§1.7  **Auto-rendered share clip.** During ceremony, the server renders a 12-second highlight reel. Per FR-VIRAL-001, exported as 1080×1920 H.264 with watermark + locale-aware hashtag prefill `#mochiwedding #virtualpet`.
...
§1.11  **No real-money wedding fee.** Per FR-LEGAL-002 §1.1. Cost is 100 Coins (earned only).

§1.12  **Wedding cooldown.** 30-day cooldown between weddings for the same pet (prevents shotgun-wedding spam).

§1.13  **AI vows (optional opt-in).** If both players consent, FR-AI-001 LLM generates personalized vows. Goes through FR-AI-002 safety. Both pets' personas referenced. Kids SKU forbidden per FR-AI-001 §1.1 (but kids SKU already blocked from wedding entirely).
...
§1.15  **Spine wedding pose.** Uses FR-ART-001 contract `wedding_pose` animation. Both pets visible in same scene.
...
§1.17  **Cross-tenant + cross-SKU locked.** Per FR-INFRA-003 + FR-AUTH-003.

§1.18  **Audit.** `social.wedding.proposed`, `social.wedding.accepted`, `social.wedding.completed { wedding_id }`, `social.wedding.ended { trigger }` per FR-OBS-001.
...
**Why 12s share clip.** TikTok-native short-form (vs FR-VIRAL-001 6-default). Wedding warrants longer.

**Why optional AI vows.** Plan §PART 3 #3 + personalisation pinnacle. Opt-in respects FR-AI-002 cost/safety.
...
**AC10.** AI vows opt-in respects FR-AI-002. Verified.
...
```typescript
describe('FR-SOCIAL-004 — wedding', () => {
...
**External:** None new.
**Internal:** FR-SOCIAL-002 (presence model), FR-VIRAL-001 (export pipeline), FR-ART-001 (wedding_pose), FR-AI-001+002 (optional vows).
**Blocks:** none.
...
| 6 | AI vows safety block | FR-AI-002 | Fallback scripted | Standard vows |
| 7 | Married_to cascade orphans | FR-LEGAL-001 cascade | Auto-null | Set null on delete |
...
**Cross-reference:** FR-VIRAL-001 share path. Plan §PART 3 viral hooks.
