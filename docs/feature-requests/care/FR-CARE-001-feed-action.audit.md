...
- **ISS-002 (error) Concurrent feed race.** Round-1 noted §1.5/§1.7 had multi-write stat mutation but did not address two simultaneous writes. **RESOLVED §10 row 4** — per-pet mutex in PetRoom (cross-ref FR-PET-003 §10 row 12).
...
- **ISS-004 (info) Pet+ rate-limit headroom.** Round-2 noted §1.14 set 60/hour but Pet+ owners with 10 pets need more. **RESOLVED §10 row 12** — 120/hour for Pet+ via FR-SUB-001 entitlement check.
...
- No real-money food respects FR-LEGAL-002.
- Audit row + analytics event differentiate `source` (ui vs co_parent_remote vs offline_reconciled) — useful for funnel work.
- Impossible-state detection inherits FR-INFRA-002 §1.9 anti-cheat.
- 90/7y retention satisfies COPPA + GDPR.

## §5 — Resolution

`score_post_revision_2: 10/10` — accept. FR-CARE-001 is the template the other CARE actions follow.
