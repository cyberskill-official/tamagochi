...
created: 2026-05-17
shipped: 2026-05-17
related_frs: [FR-PET-003, FR-PET-004, FR-ART-001, FR-CARE-001, FR-CARE-005, FR-OBS-001, FR-INFRA-002, FR-AI-001, FR-VIRAL-002]
depends_on: [FR-CARE-001]
blocks: [FR-CARE-005, FR-PET-004, FR-VIRAL-002]
effort_hours: 6
new_files:
...
risk_if_skipped: "Hug is the free zero-cost daily-engagement primitive — without it, the daily retention loop has no cost-free option for low-engagement players; first-pat onboarding (FR-PET-004 §1.5) uses the same handler so this is hard-blocking."
...
§1.16  **Idempotency.** Same shape as FR-CARE-001.

§1.17  **Source attribution.** ui / co_parent_remote.

§1.18  **First-pat reuse.** FR-PET-004 §1.5's first-pat MUST invoke this handler (with `source='first_pat'`). Diminishing-returns + cooldown bypassed for the onboarding case via a server-side bypass header.

§1.19  **No third-party gating.** Free action regardless of subscription tier — Pet+ does not unlock a "premium hug."

§1.20  **AI surface integration.** Each hug emits a `care.hug.success` event with current happiness; FR-AI-001 LLM persona MAY reference this in next dialogue line ("you've been hugging me so much today!").
...
**Why FR-PET-004 first-pat shares this handler.** Onboarding's first-pat is conceptually identical to "hug" — same animation, same +25 happiness. Reusing the handler avoids two code paths.
...
```typescript
// apps/api/src/care/__tests__/hug.spec.ts
describe('FR-CARE-003 — hug action', () => {
...
**Internal:** FR-CARE-001 (pattern), FR-PET-003 (stat apply — needs `hug_diminished` action key extension), FR-PET-004 (onboarding bypass header), FR-ART-001 (`care_pet` animation).
**Blocks:** FR-CARE-005 (streak system), FR-VIRAL-002 (Daily Drama events), FR-AI-001 (persona context).
...
| 5 | Concurrent hug + feed | Per-pet mutex | None | (FR-PET-003) |
| 6 | Co-parent broadcast race with diminishing | UX desync | Stale view | Persistent state authoritative |
| 7 | 24h cap fires while user is in pet wedding ceremony | Edge case | Wedding gives a hug bonus? | FR-SOCIAL-004 handles via separate path |
...
| 12 | AI surface dependency leaks PII | Per FR-AI-002 content safety | Filter applied |
...
**Cross-reference:** FR-PET-004 first-pat reuses this handler. FR-CARE-005 streak detection counts hug as one engagement primitive.
