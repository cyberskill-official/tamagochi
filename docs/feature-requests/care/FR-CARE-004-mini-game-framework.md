...
created: 2026-05-17
shipped: 2026-05-17
related_frs: [FR-INFRA-001, FR-INFRA-002, FR-PET-003, FR-ART-001, FR-CARE-005, FR-ECON-001, FR-OBS-001, FR-OBS-002, FR-LEGAL-002, FR-VIRAL-002, FR-B2B-004]
depends_on: [FR-INFRA-001, FR-INFRA-002, FR-PET-003]
blocks: [FR-CARE-005, FR-VIRAL-002, FR-B2B-004]
effort_hours: 14
new_files:
...
risk_if_skipped: "Mini-game is the canonical soft-currency source (plan §PART 3); without a server-validated framework, currency inflation is trivial via client score forgery. FR-CARE-005 streak system + FR-ECON-001 currency ledger both depend on this."
...
§1.2  **Per-game asset bundle.** Each mini-game lives in its own Cocos asset bundle (per FR-INFRA-001 §1.5) loaded on demand via `MiniGameLoader`. Bundle budget ≤ 2 MB per game (per FR-INFRA-001 §1.5 budget table).
...
§1.7  **Stat side-effects.** Per FR-PET-003 §1.4, mini-game outcomes affect stats:
- Win: cleanliness -3, happiness +20, energy -10.
- Loss: cleanliness -2, happiness +5, energy -8.

Stats clamped 0-100.

§1.8  **Stage gate.** Mini-games require `baby|teen|adult`. (`baby` is the gating moment for the first mini-game per FR-PET-002 §1.6.)
...
§1.14  **No real-money outcome.** Per FR-LEGAL-002 §1.1, mini-game payouts are earned-currency only. No mini-game outcome MAY: (a) determine an IAP-purchasable item; (b) consume an IAP-purchased entry ticket; (c) gate a randomised inventory pull.
...
§1.20  **Analytics taxonomy.** `mini_game.start { game_id, difficulty }`, `mini_game.finish { game_id, server_score, coin_payout, status: 'win'|'loss'|'incomplete' }`, `mini_game.score_mismatch` per FR-OBS-001.
...
**Why per-game asset bundle.** Mini-games are heavy assets (~1-2 MB each). Bundling each separately keeps cold-start fast (FR-INFRA-001 §1.5 budget) — load only the chosen game.
...
```typescript
// apps/api/src/care/mini-game/__tests__/mini-game.spec.ts
describe('FR-CARE-004 — mini-game framework', () => {
...
**External:** Cocos asset bundle infra (FR-INFRA-001).
**Internal:** FR-INFRA-002 (Colyseus broadcast), FR-PET-003 (stat apply for win/loss), FR-ART-001 (per-game assets follow art contract).
**Blocks:** FR-CARE-005 (streak system reads mini-game wins), FR-VIRAL-002 (Daily Drama micro-events), FR-B2B-004 (Techcombank financial-literacy mini-game extends framework).
...
- Randomised inventory pull as mini-game reward (per FR-LEGAL-002).
- Mini-games on grandma/egg pets.

**Cross-reference:** FR-CARE-005 reads mini-game wins for streak detection. FR-B2B-004 Techcombank tenant extends framework with savings-quest mini-game.
