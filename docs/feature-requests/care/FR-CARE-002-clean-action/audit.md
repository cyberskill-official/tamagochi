...
Clean action spec mirrors FR-CARE-001 with clean-specific deltas (cleanliness +60/+85, energy -5), bubble particles, reduce-motion respect, lower rate-limit (30/hr). 6 ISS findings resolved → 10/10.

## §2 — Round-1 findings (resolved)

- **ISS-001 (error) `clean_premium` action key not in FR-PET-003 stat-config.** Round-1: FR-CARE-002 §1.7 references `clean_premium` but FR-PET-003 §3.2 only has `clean`. **RESOLVED §7 + §11** — explicit cross-reference flagging the extension needed in FR-PET-003 §3.2.
...
- **ISS-006 (info) Pet+ rate-limit headroom.** Round-2: 30/hr too tight for multi-pet users. **RESOLVED §10 row 8** — raise to 60/hr for Pet+ via FR-SUB-001.

## §4 — Strengths preserved

- Pattern parity with FR-CARE-001 — implementer follows the proven template.
...
- Co-parent + analytics + audit + idempotency aligned with FR-CARE-001.

## §5 — Resolution

`score_post_revision_2: 10/10` — accept. FR-CARE-002 + the FR-PET-003 §3.2 `clean_premium` extension landing together.
