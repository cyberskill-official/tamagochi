...
created: 2026-05-17
shipped: 2026-05-17
related_frs: [FR-PET-003, FR-ART-001, FR-CARE-001, FR-CARE-005, FR-ECON-001, FR-OBS-001, FR-INFRA-002, FR-VIRAL-002]
depends_on: [FR-CARE-001]
blocks: [FR-CARE-005, FR-VIRAL-002]
effort_hours: 6
new_files:
...
disallowed_tools:
  ...
  - "Particle bursts without reduce-motion respect (FR-A11Y-001)"
risk_if_skipped: "Cleanliness is one of four stats — without a clean action, the stat decays unmitigated, FR-PET-002 care-gated stalls trigger constantly, and FR-CARE-005 streak system loses a daily-engagement primitive."
...
§1.2  **Stage gate.** Permitted only in `baby|teen|adult` per FR-PET-002 §1.6.

§1.3  **Ownership / co-parent check.** Owner OR `active` co-parent (FR-SOCIAL-002 P2). Else 403.

§1.4  **Soap catalogue.**
- `basic`: 5 Coins, cleanliness +60, cooldown 10 min.
- `premium` (fragrant): 20 Coins, cleanliness +85 + happiness +3, cooldown 20 min.

§1.5  **Currency spend.** Stub at P1 via FR-ECON-001 ledger.

§1.6  **Cooldown.** Per (pet, soap) Redis sliding-window; 429 with `retry_after_sec` on miss.

§1.7  **Stat apply.** `StatDecayService.apply(stats, 'clean')` per FR-PET-003 §3.1 (cleanliness +60 baseline) for `basic`; premium adds +25 cleanliness + 3 happiness via extended action key `clean_premium`.

§1.8  **Energy cost.** Clean costs energy -5 per FR-PET-003 §1.4. Energy can drop to 0; pet "sleepy" UX when energy < 20.

§1.9  **Last-seen update.** Per FR-PET-001 §1.11.
...
§1.17  **Idempotency.** Same shape as FR-CARE-001 §1.10.

§1.18  **Source attribution.** `ui` / `co_parent_remote`.

§1.19  **No real-money soap** per FR-LEGAL-002.
...
Mirrors FR-CARE-001 pattern (server-authoritative + cooldown + idempotent + audit). Differences:
...
- **Lower cooldown ceiling than feed.** Cleanliness decays slower (FR-PET-003 §1.2) so the player legitimately cleans less; 10-20 min cooldowns match the natural cadence.
...
- **Reduce-motion cap.** FR-A11Y-001 — particle bursts can trigger vestibular symptoms; reduce to 6 with fade.
...
```typescript
// apps/cocos/assets/_root/care/__tests__/CleanAction.spec.ts
describe('FR-CARE-002 §1.12 — bubble particle reduce-motion', () => {
...
**Internal:** FR-CARE-001 (template), FR-PET-003 (stat apply + clean_premium action key needs adding to FR-PET-003 §3.2 stat-config), FR-ART-001 (care_clean animation), FR-INFRA-002 (Colyseus broadcast).
**Blocks:** FR-CARE-005 (streak detection reads care_actions), FR-VIRAL-002 (Daily Drama can reference clean events).
...
| 5 | Concurrent clean + feed mutate stats out-of-order | Per-pet mutex catches | Race avoided | (covered by FR-PET-003 §10 row 12) |
| 6 | Premium soap accidentally surfaced as IAP | FR-LEGAL-002 lint | Build blocked | Catalogue field guarded by lint |
...
**Cross-reference:** FR-CARE-003/004 follow same pattern. FR-PET-003 §3.2 needs an extension `clean_premium` action key (-5 energy, +85 cleanliness, +3 happiness) — flagged in §7.
