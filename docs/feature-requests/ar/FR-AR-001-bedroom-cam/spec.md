...
created: 2026-05-17
shipped: 2026-05-17
related_frs: [FR-INFRA-001, FR-PET-001, FR-PET-002, FR-ART-001, FR-VIRAL-001, FR-LEGAL-001, FR-LEGAL-003, FR-A11Y-001, FR-OBS-001]
depends_on: [FR-INFRA-001, FR-PET-002, FR-ART-001]
blocks: [FR-VIRAL-001]
effort_hours: 14
new_files:
...
disallowed_tools:
  - "AR placement on kids SKU without parental gate (FR-LEGAL-003 §1.4)"
  - "Sharing captured video via deep link to external apps for under-13 (FR-LEGAL-003 external-link gate)"
...
risk_if_skipped: "Plan §PART 3 viral hook #2 — AR placement is the documented TikTok-native virality wedge. Without it, the 'Bedroom Cam' moment that drives organic UGC is missing; FR-VIRAL-001 export pipeline has nothing to export."
...
§1.8  **Save to camera roll.** Captured video saved to the OS camera roll (with explicit user consent gate per platform). Returns the file URI to the Cocos layer for FR-VIRAL-001 share-intent.
...
§1.10  **Kids SKU disabled.** Per FR-LEGAL-003 §1.4, the AR-share path goes outside the app (camera roll → user's other apps). Under-13 must use parental gate (math problem). At P1, the kids SKU **disables the AR feature entirely** — the entry point is hidden. Re-enable in P3 as part of the parental-controlled gate flow.

§1.11  **Stage gate.** AR placement requires `stage ∈ {teen, adult}` (per FR-PET-002 §1.6) — eggs/babies cannot be ARed (UX rationale: AR is a "we go places together" intimate experience, fits maturity).

§1.12  **AR session telemetry.** `ar.session.started`, `ar.session.placed { plane_count, time_to_place_ms }`, `ar.session.recorded { duration_sec }`, `ar.session.failed { reason }` per FR-OBS-001.

§1.13  **Performance budget.** AR rendering MUST sustain ≥ 30 fps on the FR-INFRA-001 §1.20 baseline VN 1 GB Android device. Falls below → graceful degrade (lower Spine sample rate; warn user with "Photo Studio mode might work better").
...
```typescript
// apps/cocos/assets/_root/ar/__tests__/PetArPlacer.spec.ts
describe('FR-AR-001 — PetArPlacer', () => {
...
**Internal:** FR-INFRA-001 (Cocos scaffold), FR-PET-002 (stage gate), FR-ART-001 (Spine `ar_greet`).
**Blocks:** FR-VIRAL-001 (TikTok export pipeline reads AR video file URI).
...
**Cross-reference:** FR-VIRAL-001 consumes the captured video URI for share-intent. FR-A11Y-001 reduce-motion applies.
