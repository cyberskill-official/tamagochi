...
created: 2026-05-17
shipped: 2026-05-17
related_frs: [FR-B2B-001, FR-B2B-002, FR-B2B-003, FR-CARE-001, FR-LEGAL-001, FR-AUTH-001, FR-AUTH-002, FR-ECON-001, FR-OBS-001]
depends_on: [FR-B2B-003, FR-CARE-001]
blocks: []
effort_hours: 14
new_files:
...
§1.1  **Tenant slug `viettel`.** Per FR-B2B-001.

§1.2  **Audience.** Adult Viettel subscribers (13+ default). Family-plan members may sub-create kid profiles via FR-SUB-002.

§1.3  **Bundle ID.** `world.viettel.pet-of-viettel` (Apple) / `vn.com.viettel.petofviettel` (Android).

§1.4  **Viettel SSO.** Viettel ID OAuth 2.0 + PKCE. Per FR-AUTH-002 Zalo-like flow.
...
§1.13  **No real-money out from pet.** Per FR-LEGAL-002 — pet rewards never convert outside the Viettel loyalty channel.

§1.14  **Audit retention.** 7-year (telco regulation alignment).

§1.15  **Cross-tenant restrictions.** Same as FR-B2B-004 §1.12 + §1.13 — no trade / co-parent with consumer.

§1.16  **Parental consent for under-13.** If a Viettel adult subscribes their child (≤ 12), parental consent via FR-AUTH-003.

§1.17  **DPO review of Viettel quests + AI dialogue.** Per FR-LEGAL-001.

§1.18  **Per-tenant Viettel analytics workspace.** Per FR-OBS-001 §1.15.

§1.19  **Family tier interop.** Family-plan Viettel subscribers may use FR-SUB-002 Family tier for child profiles under the same Viettel account.

§1.20  **Analytics.** `viettel.sso.callback`, `viettel.topup.received`, `viettel.daily_login.streak_7`, `viettel.data_discount.applied` per FR-OBS-001.
...
export class TopupTriggerService {
  async handleTopupWebhook(body: { viettel_id: string; sim_card_hash: string; topup_amount_vnd: number; topped_up_at: string }, signature: string) {
    ...
    // Auto-feed pet (or queue food via FR-CARE-001 if pet at hunger=100)
...
**Internal:** FR-B2B-001 manifest, FR-B2B-002 partition, FR-B2B-003 console, FR-CARE-001 feed action, FR-AUTH-001 + FR-AUTH-002 (Vietnamese telco SSO model).
**Blocks:** none.
...
**Cross-reference:** FR-B2B-001 manifest; FR-B2B-002 partition; FR-CARE-001 feed; FR-AUTH-002 Vietnamese-telco SSO pattern.
