...
created: 2026-05-17
shipped: 2026-05-17
related_frs: [FR-B2B-001, FR-B2B-002, FR-AUTH-001, FR-LEGAL-001, FR-OBS-001, FR-INFRA-003, FR-B2B-004, FR-B2B-005]
depends_on: [FR-B2B-001, FR-B2B-002, FR-AUTH-001]
...
disallowed_tools:
  ...
  - "Theme upload bypassing signature (FR-B2B-001 §1.14)"
...
§1.4  **Quest CMS.** Operator authors quests via web UI: title, description, trigger (sign-in, save 100K VND, etc.), reward (Coins). Saved to per-tenant CMS API consumed by FR-B2B-001 manifest.

§1.5  **KPI dashboard.** Read-only dashboard: DAU/MAU, retention curves, IAP revenue, top quests by completion. Powered by FR-OBS-001 per-tenant analytics workspace.

§1.6  **Audit log view.** Read-only audit timeline per FR-LEGAL-001 + FR-OBS-002 — DSR requests, ban events, theme uploads, etc.

§1.7  **Billing tab.** Per FR-B2B-002 SLA + tier — monthly invoice, MAU usage, payment status.
...
§1.13  **No tenant cross-access.** Operators see only their tenant. Cross-tenant requires DPO grant per FR-B2B-002 §1.8.

§1.14  **Audit retention.** 7 years.

§1.15  **Console hosted at admin.petos.tamagochi.app.** Separate hostname from consumer game.

§1.16  **DPO review of quest copy.** Per FR-AI-002 + FR-LEGAL-001 — kid-tenant quest copy requires DPO sign-off before live.
...
§1.20  **Analytics.** `petos.operator.login`, `petos.theme.uploaded`, `petos.quest.changed`, `petos.dsr.export_initiated` per FR-OBS-001.
...
**Why DPO review of quest copy for kids.** FR-LEGAL-001 — kid-tenant content must pass review.
...
```typescript
describe('FR-B2B-003 — PetOS console', () => {
...
**Internal:** FR-B2B-001 (manifest base), FR-B2B-002 (tenant partition), FR-AUTH-001 (auth), FR-OBS-001 (per-tenant analytics), FR-LEGAL-001 (DPO).
**Blocks:** FR-B2B-004/005.
...
**Cross-reference:** FR-B2B-001 manifest; FR-B2B-002 partition; FR-B2B-004 Techcombank; FR-B2B-005 Viettel.
