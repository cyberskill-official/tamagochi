...
created: 2026-05-17
shipped: 2026-05-17
related_frs: [FR-INFRA-001, FR-OBS-001, FR-LEGAL-001, FR-I18N-002, FR-AI-002, FR-VIRAL-005, FR-AUTH-001]
...
disallowed_tools:
  ...
  - "Server-side hardcoded copy (use FR-OBS-001 schema-keyed locale)"
...
§1.5  **Per-feature namespacing.** Keys are namespaced: `care.feed.success`, `petpair.invite.received`, `streak.milestone.30`, etc. Aligns with FR-OBS-001 §1.4 event taxonomy.
...
§1.9  **RTL stub.** Future Arabic / Hebrew support deferred but stub'd: `I18nService.isRtl(locale)` returns boolean, UI layouts read this. Initial wave is all LTR; RTL added in future FR.

§1.10  **Locale detection.** Detected from FR-AUTH-001 `region_of_record`, then user override via Settings, then OS locale, then 'en' fallback.

§1.11  **Missing-key fallback.** Cocos `I18nService` returns key string surrounded by `~missing~` brackets if key not in current locale; falls back to EN if EN has it; emits `i18n.missing_key { key, locale }` event to FR-OBS-001 for triage.
...
§1.18  **Tenant-aware copy override.** B2B tenants (FR-B2B-001) may override specific keys per tenant + locale. Override path: `i18n/tenants/<slug>/<locale>/*.json`.

§1.19  **Audit + analytics.** `i18n.locale_changed`, `i18n.missing_key`, `i18n.font_fallback_applied` per FR-OBS-001.
...
**Internal:** FR-INFRA-001 (Cocos asset bundle), FR-AUTH-001 (region_of_record), FR-OBS-001 (missing-key event).
**Blocks:** FR-I18N-002 (payment localisation), FR-A11Y-001 (locale-aware A11Y).
...
**Cross-reference:** FR-I18N-002 payment localisation; FR-A11Y-001 reads font fallback.
