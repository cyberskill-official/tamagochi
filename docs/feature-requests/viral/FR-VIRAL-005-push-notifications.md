...
created: 2026-05-17
shipped: 2026-05-17
related_frs: [FR-CARE-005, FR-LEGAL-001, FR-LEGAL-003, FR-AUTH-001, FR-AUTH-003, FR-SUB-002, FR-INFRA-003, FR-OBS-001, FR-PET-002, FR-PET-008, FR-SOCIAL-002]
depends_on: [FR-CARE-005, FR-LEGAL-001]
blocks: []
effort_hours: 10
new_files:
...
disallowed_tools:
  - "Engagement-pushing for under-13 (COPPA-2025 restriction per FR-LEGAL-001 §1.5(e))"
  - "Push during sleep hours (local 22:00-07:00 default; 20:00-08:00 kids)"
  - "Marketing pushes without opt-in"
  - "Cross-tenant push leak"
risk_if_skipped: "Plan §PART 3 retention mechanics — push is the canonical re-engagement surface. Without it, daily-engagement rate drops 30-50%. FR-CARE-005 streak notifications + FR-SOCIAL-002 PetPair receipts + FR-PET-008 grandma warnings all require this."
...
- **grandma_warning** (default ON): per FR-PET-008.
...
- Kids SKU: max 1 push per day per user (engagement-pushing restriction per FR-LEGAL-001 §1.5(e)).

§1.4  **Parental override.** Per FR-SUB-002 — Family tier parent can disable per-child push categories from dashboard.
...
§1.7  **Localised copy.** Title + body MUST come from a locale string bundle keyed by `title_key` per FR-I18N-001. No raw strings server-side.
...
§1.10  **No engagement-pushing for under-13.** Per FR-LEGAL-001 §1.5(e). For kids SKU, only categories: `breeding_hatched`, `grandma_warning`, `petpair_receipt` (parent-initiated only). NO `care_reminder` engagement nudges.

§1.11  **Push token lifecycle.** Tokens auto-revoked on:
- App uninstall (FCM/APNS feedback).
- User sign-out.
- Account deletion (FR-LEGAL-001 §1.9).
...
§1.13  **Deep linking.** Each push includes a deep-link URL: `tamagochi://pet/<id>` (opens game to that pet). Per FR-LEGAL-003 §1.4 — kid pushes deep-link to in-app only, never external.

§1.14  **Time-zone handling.** Player's `region_of_record` (FR-AUTH-001) + IANA tz lookup. DST-aware.

§1.15  **Cross-tenant scoping.** Push per-tenant; never cross-leak.

§1.16  **Tenant override.** B2B tenants (FR-B2B-001) may have custom push category sets + Pet+/Family-tier overrides — defined in tenant config.
...
§1.20  **Analytics.** `push.scheduled`, `push.suppressed { reason }`, `push.delivered`, `push.opened`, `push.failed { reason }`, `push.token.registered`, `push.token.invalidated` per FR-OBS-001.
...
**Why deep-link in-app for kids.** FR-LEGAL-003 §1.4 — out-of-app for under-13 requires parental gate. Push deep-link goes back to game.
...
**Internal:** FR-CARE-005 streak notifications, FR-LEGAL-001 COPPA restrictions, FR-AUTH-001 region, FR-SUB-002 parental disable.
**Blocks:** none.
...
| 9 | Deep link to external URL for kids | FR-LEGAL-003 gate | Blocked | Validation |
...
**Cross-reference:** FR-CARE-005 streak, FR-PET-008 grandma, FR-SOCIAL-002 PetPair receipt, FR-LEGAL-001 COPPA, FR-SUB-002 parental disable.
