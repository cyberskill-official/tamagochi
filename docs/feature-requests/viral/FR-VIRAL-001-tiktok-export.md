...
created: 2026-05-17
shipped: 2026-05-17
related_frs: [FR-AR-001, FR-PET-002, FR-ART-001, FR-LEGAL-003, FR-AUTH-003, FR-VIRAL-002, FR-VIRAL-003, FR-SOCIAL-004, FR-OBS-001]
depends_on: [FR-AR-001]
blocks: [FR-VIRAL-002, FR-VIRAL-003]
effort_hours: 10
new_files:
...
disallowed_tools:
  - "External-link share on kids SKU without parental gate (FR-LEGAL-003)"
...
risk_if_skipped: "Plan §PART 3 viral hook #5 + plan §PART 7 GTM — TikTok-native exports are the primary organic acquisition lever. Without this pipeline, FR-VIRAL-002 (Daily Drama) + FR-VIRAL-003 (generative pet) + FR-SOCIAL-004 (wedding) have no share surface."
...
§1.2  **6-second default.** Auto-generated clips (Daily Drama, evolution) default to 6 seconds. AR Bedroom Cam clips (FR-AR-001) extend up to 15 seconds. Configurable per export call.
...
§1.9  **Kids-SKU restriction.** Per FR-LEGAL-003 + FR-AUTH-003, the Share button MUST be hidden on the kids SKU. Kids cannot share externally without parental approval; P3+ feature flag enables parental-approved sharing.
...
§1.12  **Server-side share log retention.** `share_tracking` rows retained 90 days for analytics + privacy. Player can delete their share history via FR-LEGAL-001 DSR.
...
§1.15  **Caption locale.** Hashtag list MUST localise per FR-I18N-001:
...
§1.18  **Captioning at P1 → none.** Auto-generated captions ("Mochi feeling happy!") are deferred to P2+ once Content Safety classifier (FR-AI-002) approves caption variety.
...
§1.20  **Analytics taxonomy.** `viral.share.invoked`, `viral.share.link_click`, `viral.export.failed { reason }` per FR-OBS-001 §1.4.
...
**Why kids SKU hidden.** External-link gate complexity per FR-LEGAL-003.
...
**Why no captions at P1.** Plan §PART 3 — auto-generated captions need FR-AI-002 review. Punting reduces P1 scope.
...
export class VideoExporter {
  async export(opts: { input_video_uri: string; duration_sec: number; locale: string }): Promise<{ output_uri: string; watermarked: true }> {
    if (opts.duration_sec > 15) throw new Error('FR-VIRAL-001 §1.2 max 15s');
...
export class ShareIntent {
  async invoke(opts: { video_uri: string; caption: string; source: 'ar'|'daily_drama'|'evolution'|'wedding' }): Promise<{ invoked: true; share_link_id?: string }> {
    if (await this.isKidsSku()) throw new Error('FR-VIRAL-001 §1.9 kids cannot share');
...
```typescript
// apps/cocos/assets/_root/viral/__tests__/VideoExporter.spec.ts
describe('FR-VIRAL-001 — exporter', () => {
...
**Internal:** FR-AR-001 (provides input video URI), FR-PET-002 (evolution share moment), FR-LEGAL-003 (kids gate), FR-I18N-001 (locale).
**Blocks:** FR-VIRAL-002 (Daily Drama share), FR-VIRAL-003 (generative pet share), FR-SOCIAL-004 (wedding share).
...
**Cross-reference:** Closes P1 visual viral loop. FR-VIRAL-002 / 003 / FR-SOCIAL-004 all use this export pipeline.
