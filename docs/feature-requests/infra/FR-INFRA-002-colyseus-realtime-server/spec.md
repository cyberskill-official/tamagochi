...
created: 2026-05-17
shipped: 2026-05-17
related_frs: [FR-INFRA-001, FR-INFRA-003, FR-AUTH-001, FR-PET-001, FR-CARE-001, FR-CARE-002, FR-CARE-003, FR-CARE-004, FR-SOCIAL-001, FR-SOCIAL-002, FR-SOCIAL-003, FR-SOCIAL-004, FR-AI-001, FR-VIRAL-002, FR-OBS-002, FR-B2B-002]
depends_on: [FR-INFRA-001]
blocks: [FR-PET-001, FR-CARE-001, FR-CARE-002, FR-CARE-003, FR-CARE-004, FR-SOCIAL-001, FR-SOCIAL-002, FR-SOCIAL-003, FR-SOCIAL-004, FR-AI-001, FR-VIRAL-002, FR-OBS-002]
effort_hours: 12
new_files:
...
disallowed_tools:
  ...
  - "Server-side `Math.random()` for state-affecting outcomes — use crypto RNG only with audit trail (FR-LEGAL-002 §1)"
  - "Cross-tenant room joining (tenant_id partition enforced server-side per FR-B2B-002)"
...
§1.15  **Per-tenant rate-limit budget.** Per FR-B2B-002, each tenant MUST have its own rate-limit budget; consumer tenant `mochi` budget separate from any B2B tenant. Budgets configurable via `limits.yaml` at deploy time.
...
§1.17  **No `Math.random()` for state outcomes.** Per FR-LEGAL-002, all randomised state-affecting outcomes MUST use `crypto.randomBytes()` seeded calls routed through the `DropRateService` (FR-LEGAL-002 §6 implementation). A custom ESLint rule MUST flag `Math.random()` in `apps/realtime/src/rooms/` + `apps/realtime/src/state/`.
...
**Why `crypto.randomBytes()` not `Math.random()`.** `Math.random()` is not cryptographically random and on Node 16+ the PRNG seed is shared across worker threads. For randomised state outcomes that affect monetised inventory (per FR-LEGAL-002 audit trail), `crypto.randomBytes()` is the only acceptable source.
...
describe('FR-INFRA-002 §1.8 — rate limiter', () => {
...
export class PetRoom extends _BaseTenantRoom<RoomState> {
  maxClients = 2;        // 1 owner + 1 co-parent per FR-SOCIAL-002
...
**Internal:**
- FR-INFRA-001 (TurboRepo + TypeScript baseline).
- FR-INFRA-003 (Supabase Postgres for state flush, lands next).

**Blocks:** FR-PET-001, FR-CARE-001..005, FR-SOCIAL-001..004, FR-AI-001, FR-VIRAL-002, FR-OBS-002.
...
| 7 | Colyseus 0.16.x → 0.17.x major upgrade breaks schema | Vendor changelog | Future tech debt | Upgrade FR with regression test; pin until then |
...
**Cross-reference:** This FR is the substrate for every multi-actor gameplay FR. FR-SOCIAL-002 (PetPair co-parent), FR-SOCIAL-003 (trade), FR-SOCIAL-004 (wedding) all require this room scaffold.
