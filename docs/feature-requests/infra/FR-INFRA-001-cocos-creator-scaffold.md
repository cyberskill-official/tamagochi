...
created: 2026-05-17
shipped: 2026-05-17
related_frs: [FR-LEGAL-003, FR-INFRA-002, FR-INFRA-003, FR-ART-001, FR-PET-001, FR-AUTH-001, FR-AR-001, FR-I18N-001, FR-A11Y-001, FR-B2B-001]
depends_on: []
blocks: [FR-INFRA-002, FR-INFRA-003, FR-ART-001, FR-PET-001, FR-AR-001, FR-I18N-001, FR-A11Y-001, FR-B2B-001]
effort_hours: 14
new_files:
...
risk_if_skipped: "Without a pinned, reproducible Cocos scaffold, every downstream visual / gameplay FR will sit on a moving target; build-size budget overruns kill emerging-market mobile-web reach (plan §PART 4 cites 5-15 MB ceiling)."
audience_age_gate: "any"
---

## §1 — Description (BCP-14 normative)

§1.1  **Engine pin.** The project MUST use **Cocos Creator 3.8.x** (latest 3.8 LTS at scaffold time, pinned to a specific patch in `apps/cocos/settings/v2/packages/engine.json` with field `version: "3.8.4"` — exact version locked, not `^3.8` or `~3.8`). Major or minor upgrades MUST be a deliberate FR amendment with regression testing per §10 row 7.
...
```
The loader MUST honour `tenantSlug` so that per-tenant theme bundles (PetOS B2B per FR-B2B-001) can be loaded from `<cdnPrefix>/<tenantSlug>/<bundleName>` without touching gameplay code.

§1.7  **Plugins pinned.** The following Cocos plugins MUST be installed at scaffold time with locked versions: `@cocos/spine` (Spine 2D — required by FR-ART-001); `lottie-web` (Lottie UI micro-animation); `howler` (cross-platform audio, plan §PART 5). Each plugin's version MUST be recorded in `apps/cocos/package.json` `dependencies` with a `=` prefix (exact match), NOT `^` or `~`.
...
§1.16  **No Cocos analytics / telemetry phoning home.** The Cocos Editor and runtime ship with optional analytics SDKs that send build / runtime data to Chukong/Cocos servers. These MUST be disabled in `apps/cocos/settings/v2/packages/builder.json` (`analyticsEnabled: false`). This is a hard requirement for the kids SKU per FR-LEGAL-001 §1.5(a) (no behavioural SDKs in kids binary).

§1.17  **Source-map handling.** Web builds MUST emit source maps to a separate `*.map` file (not inlined). Source maps MUST NOT be uploaded to the public CDN; they MUST be uploaded to Sentry (per FR-OBS-001) only. The web build script MUST verify no `.map` file leaked to `apps/cocos/build/web-mobile/`.

§1.18  **`Fastfile` lanes.** `apps/cocos/fastlane/Fastfile` MUST define lanes `build_kids`, `build_standard`, `submit_kids_testflight`, `submit_standard_testflight`, `submit_kids_internal`, `submit_standard_internal`. The kids lanes MUST invoke `scripts/legal/inspect-kids-binary.mjs` (from FR-LEGAL-003) before any submission action.
...
assets/
  ...
  mini-games/             — per-game folder, each loaded as its own bundle (FR-CARE-004)
  ui/                     — global UI atlas, Lottie micro-anim sources
  audio/                  — Howler.js entry; music loops + SFX
  i18n/                   — per-locale string bundles (FR-I18N-001 deliverable)
  tenants/<tenant-slug>/  — per-tenant theme override bundle (FR-B2B-001 deliverable)
```
...
**Why pin Cocos to 3.8.4 (or whatever 3.8 patch is current) and not `^3.8`.** Cocos minor releases have shipped breaking API changes within the same major (e.g. 3.6 → 3.7 changed the asset-bundle JSON shape). A pin freezes the surface area; an upgrade FR forces a regression pass.
...
**Why tenant-slug in the loader signature already at P0.** FR-B2B-001 (multi-tenant client) lands in P4. But the loader API surface area touches every visual feature in P1-P3. Threading the slug from day zero makes B2B integration a config change in P4 rather than a refactor of every loader call.
...
**Why reproducible builds.** A reproducible build is the difference between "we shipped vYYYY-MM-DD-abc1234" and "we shipped *something* yesterday and we're trying to figure out what." Reproducibility also enables `pnpm legal:export <out.zip>` (FR-LEGAL-001 §1.13) to include cryptographic build proofs.
...
if (failures.length) {
  console.error('FR-INFRA-001 §1.5 — bundle-budget failures:');
...
describe('FR-INFRA-001 §1.4 — build target flag', () => {
...
describe('FR-INFRA-001 §1.6/§1.20 — asset bundle loader', () => {
...
// 3) Post-build: kids-binary inspection (FR-LEGAL-003 §1.10).
...
- Fastlane 2.219+.
- Cloudflare R2 (web CDN target for FR-B2B-001 theme bundles).

**Internal:** none (root infra FR).

**Blocks:** FR-INFRA-002, FR-INFRA-003, FR-ART-001 (Spine pipeline), FR-PET-001 (pet entity needs Cocos node graph), FR-AR-001 (AR plugin loads here), FR-I18N-001, FR-A11Y-001, FR-B2B-001 (tenant bundle loader piggy-backs on §1.6).
...
FR-INFRA-001 §1.5 — bundle-budget failures:
...
- **OQ-1 (resolved):** Cocos 3.8.x vs 3.9 if 3.9 releases mid-P0? → §1.1 — exact patch pin; upgrade is its own FR.
...
|---|---|---|---|---|
| 1 | Bundle budget exceeded after a designer adds a high-res texture | `assert-bundle-budget` CI check | PR blocked | Compress texture; re-PR; or escalate budget revision via FR amendment |
| 2 | Cocos 3.8.x patch ships a breaking change in asset bundle JSON shape | CI build fails on regression suite | Build pipeline broken | Pin to prior patch; raise upgrade FR; do not auto-upgrade |
...
| 7 | Cocos 3.x → 4.x major upgrade lands and the ecosystem starts moving | Vendor watch + cocos.io changelog | Future technical debt | Author an upgrade FR with regression test suite + rollback plan |
...
- `analyticsEnabled: true` in builder.json — never (per FR-LEGAL-001).
- Source maps on public CDN — Sentry only.

**Cross-reference:** This FR is the substrate every visual / gameplay FR builds on. Without it pinned, every later FR sits on a moving target.
