...
created: 2026-05-17
shipped: 2026-05-17
related_frs: [FR-LEGAL-001, FR-INFRA-001, FR-INFRA-002, FR-AUTH-001, FR-AUTH-002, FR-AUTH-003, FR-OBS-001, FR-PET-001, FR-ECON-001, FR-B2B-002]
depends_on: [FR-INFRA-001, FR-LEGAL-001]
blocks: [FR-AUTH-001, FR-AUTH-002, FR-AUTH-003, FR-PET-001, FR-OBS-001, FR-ECON-001]
effort_hours: 12
new_files:
...
disallowed_tools:
  ...
  - "Single Supabase project for both kids and standard SKU (FR-LEGAL-001 §1.12 forbids commingling)"
...
risk_if_skipped: "Without RLS templates + tenant partition + KMS-encrypted backups + dedicated kids project, the data layer fails COPPA-2025 + PDPL audit on day one; B2B PetOS (FR-B2B-002) would require a database refactor at P4."
audience_age_gate: "any"
---

## §1 — Description (BCP-14 normative)

§1.1  **Two Supabase projects.** Per FR-LEGAL-001 §1.12, the kids SKU and 13+ SKU MUST use **two dedicated Supabase projects**: `tamagochi-standard` and `tamagochi-kids`. Each has its own Postgres instance, Storage bucket, Auth users table, Edge Functions deployment. Cross-project data sharing is forbidden except via the explicit graduation flow (§1.13). Project IDs MUST be recorded in `infra/supabase/standard/config.toml` and `infra/supabase/kids/config.toml`.

§1.2  **Postgres pin.** Both projects MUST run **Postgres 15** managed by Supabase. Version upgrades are deliberate FR amendments.
...
§1.7  **Anon key with RLS-enforced read scope.** The Supabase `anon` key MAY be embedded in the Cocos client and is used by unauthenticated affordances (public drop-rate JSON read per FR-LEGAL-002, public privacy policy read). RLS templates MUST ensure the `anon` role cannot read any player or parental-consent data.
...
§1.12  **Audit log table.** A `audit_log` table MUST capture every privileged data access: SELECTs of `parental_consent`, `dsr_tickets`, `breach_drafts`, `coppa_age_gate_log` (per FR-LEGAL-001). Logged columns: `who, when, what_table, what_keys, tenant_id, source_request_id`. Retention 7 years. Read-only after insert.
...
§1.14  **Edge Functions for parental-consent webhook.** A Deno-runtime Edge Function `parental-consent-webhook` MUST accept Safe Harbor vendor callbacks (PRIVO / SuperAwesome kWS per FR-LEGAL-001 §1.6), verify signature, and update `parental_consent` rows. The function MUST be deployed in the kids project only.

§1.15  **Breach-draft flush Edge Function.** A `breach-draft-flush` Edge Function MUST nightly snapshot the `breach_drafts` table to a regulator-accessible read-only mirror table for audit replay. This satisfies FR-LEGAL-001 §1.4's 72-hour-window audit requirement.

§1.16  **Storage buckets.** Storage buckets MUST be created with documented policies: `pets/` (avatar + custom-design uploads — public-read-after-moderation), `legal/` (DSR evidence + breach drafts — DPO-only-read), `tenants/<slug>/` (B2B theme assets per FR-B2B-001 — anon-read-by-slug). Per-bucket file-size limits enforced.

§1.17  **Realtime feature opt-out.** Supabase's built-in Realtime is **disabled by default** — tamagochi uses Colyseus (FR-INFRA-002) for stateful real-time. Supabase Realtime MAY be enabled per-table for non-state propagation (e.g. tenant-config push to clients) with explicit FR amendment.
...
**Why an audit_log table even for SELECTs.** Most teams audit only writes. For COPPA + PDPL, *reads* of sensitive data (parental_consent, dsr_tickets) are also audit-relevant — a regulator may ask "who read this child's data on what date." 7-year retention parallels FR-LEGAL-002 §1.4 receipts.
...
**Why disable Supabase Realtime by default.** Colyseus is the canonical real-time channel. Two real-time systems doubles the failure modes; using Supabase Realtime for incidental push (tenant-config) is fine, but only with explicit FR.
...
-- tenant-scoped read (e.g. pets-of-my-tenant only — used by FR-PET-001)
-- example application:
-- create policy "pets tenant-scoped read"
--   on public.pets for select
--   using (tenant_id = current_setting('app.tenant_id', true));

-- dpo-only (per FR-LEGAL-001 §3.4)
...
begin
  if new.tenant_id is null or new.tenant_id = '' then
    raise exception 'FR-INFRA-003 §1.4 — tenant_id is required';
  end if;
  return new;
end;
$$ language plpgsql;
...
describe('FR-INFRA-003 §1.4 — tenant partition', () => {
...
**Internal:**
- FR-INFRA-001 (TurboRepo workspace exists).
- FR-LEGAL-001 (DPO + Safe Harbor + audit tables drive the schema).

**Blocks:** FR-AUTH-001, FR-AUTH-002, FR-AUTH-003, FR-PET-001 (pets table), FR-OBS-001 (analytics tables), FR-ECON-001 (currency ledger), FR-B2B-002 (tenant partition consumer).
...
- Supabase Realtime on a sensitive table without FR amendment.

**Cross-reference:** This FR completes the P0 INFRA trio (INFRA-001 + 002 + 003) and unblocks AUTH (which depends on Supabase Auth) + every gameplay FR that persists state.
