...
created: 2026-05-17
shipped: 2026-05-17
related_frs: [FR-ECON-001, FR-INFRA-002, FR-INFRA-003, FR-OBS-001, FR-SOCIAL-003, FR-PET-007, FR-CARE-004, FR-LEGAL-001]
depends_on: [FR-ECON-001, FR-INFRA-002, FR-OBS-001]
blocks: [FR-B2B-002]
effort_hours: 14
new_files:
...
risk_if_skipped: "Plan §PART 4 + plan §PART 10 — at 100K MAU + active trade economy, anti-cheat is critical. Without comprehensive auditing, FR-SOCIAL-003 trade + FR-ECON-001 currency + FR-PET-007 breeding are exploitable. FR-B2B-002 enterprise tenants require this for SLA."
...
- **L1 Auth gate** (FR-AUTH-001 JWT) — caught at sign-in.
- **L2 Message signing** (FR-INFRA-002 §1.7 HMAC) — caught per-message.
- **L3 Rate limiting** (FR-INFRA-002 §1.8) — caught per-volume.
- **L4 Impossible-state guard** (FR-INFRA-002 §1.9) — caught per-action.
- **L5 Ledger reconciliation** (FR-ECON-001 §1.12) — caught daily.
- **L6 This FR — ban-decision orchestrator** + audit dashboard.

§1.2  **Impossible-state event sources.** Aggregates events from:
- Negative stat after care (FR-PET-003).
- Feed at hunger=100 (FR-CARE-001).
- Trade with grandma pet (FR-SOCIAL-003).
- Breeding non-adult pets (FR-PET-007).
- Score mismatch in mini-game (FR-CARE-004).
- Currency negative or above grant (FR-ECON-001).
- Cross-tenant operation (FR-INFRA-003).
- Cross-SKU operation (FR-AUTH-003).
- HMAC signature invalid (FR-INFRA-002).
...
- Cannot sign in (FR-AUTH-001 returns 403 `account.banned`).
...
§1.7  **Daily ledger reconciliation.** Per FR-ECON-001 §1.12, scheduled job recomputes balances + detects drift > 0.1%. Drift alerts via Sentry → DPO review.
...
§1.20  **Analytics + audit.** `security.flag.incremented`, `security.ban.applied { tier }`, `security.appeal.submitted`, `security.appeal.resolved { verdict }`, `security.false_positive.recovered`, `ledger.reconciliation.drift_detected` per FR-OBS-001.
...
```typescript
describe('FR-OBS-002 — anti-cheat', () => {
...
**Internal:** FR-ECON-001 (ledger drift), FR-INFRA-002 (HMAC, rate-limit), FR-AUTH-001 (ban gate at sign-in), FR-PET-008 (grandma archive), FR-LEGAL-001 (DPO + audit).
**Blocks:** FR-B2B-002 (B2B tenants depend on anti-cheat SLA).
...
**Cross-reference:** FR-ECON-001 ledger drift; FR-INFRA-002 HMAC + rate-limit; FR-B2B-002 multi-tenant requirement.
