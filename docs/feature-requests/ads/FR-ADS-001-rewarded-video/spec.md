...
created: 2026-05-17
shipped: 2026-05-17
related_frs: [FR-LEGAL-003, FR-ECON-001, FR-CARE-005, FR-AUTH-003, FR-SUB-001, FR-ADS-002, FR-INFRA-003, FR-OBS-001, FR-LEGAL-001]
depends_on: [FR-ECON-001, FR-LEGAL-003]
blocks: [FR-ADS-002]
effort_hours: 8
new_files:
...
disallowed_tools:
  - "Interstitial ads (FR-LEGAL-003 + kid-retention damage)"
  - "Behavioural ads on kids SKU (FR-ADS-002 contextual-only)"
  - "Ads for Pet+ subscribers (FR-SUB-001 ad-free benefit)"
...
§1.1  **Only rewarded video.** No interstitials (per FR-LEGAL-003 + kid-app guideline). No banners. No native ads. Only rewarded video.

§1.2  **Mediation waterfall.** **LevelPlay (IronSource) primary** + **AppLovin MAX fallback**. Per FR-LEGAL-001 §1.5(a) — only on standard SKU.

§1.3  **Reward.** Each completed rewarded view grants 50 Coins via FR-ECON-001 ledger (`ref = 'ads.rewarded:<view_id>'`).
...
§1.8  **No Pet+ ads.** Pet+ subscribers (FR-SUB-001 §1.2) have "ad-free" benefit. Endpoint returns 403 `ads.pet_plus_disabled` if attempted.

§1.9  **Kids SKU forbidden.** This FR is 13+ only. Kids contextual-only ads via FR-ADS-002.
...
§1.19  **No ad before onboarding.** Per FR-PET-004 §1.7 — no ads during onboarding flow.

§1.20  **Analytics.** `ads.rewarded.requested`, `ads.rewarded.shown`, `ads.rewarded.completed`, `ads.rewarded.skipped`, `ads.rewarded.cap_hit`, `ads.rewarded.failed { reason }` per FR-OBS-001.
...
**Why daily 8 cap.** Anti-grind. 8 × 50 = 400 Coins/day from ads ≈ same as 2 surprise-egg openings. Coherent with the FR-PET-006 economy.
...
**Internal:** FR-ECON-001 ledger, FR-LEGAL-003 SKU + ad policy, FR-SUB-001 (Pet+ ad-free).
**Blocks:** FR-ADS-002 (kids contextual gate).
...
| 4 | Pet+ entitlement stub returns wrong | FR-SUB-001 wired | Ads to Pet+ accidentally | Cache invalidation |
...
| 8 | Behavioural ad on kids accidentally | FR-ADS-002 catches | COPPA risk | Audit |
...
**Cross-reference:** FR-ADS-002 contextual kid gate; FR-SUB-001 ad-free benefit.
