...
created: 2026-05-17
shipped: 2026-05-17
related_frs: [FR-INFRA-002, FR-INFRA-003, FR-CARE-001, FR-CARE-002, FR-CARE-004, FR-PET-006, FR-PET-007, FR-ECON-002, FR-ECON-003, FR-SUB-001, FR-ADS-001, FR-VIRAL-004, FR-OBS-001, FR-OBS-002, FR-LEGAL-002, FR-B2B-002]
depends_on: [FR-INFRA-003]
blocks: [FR-ECON-002, FR-ECON-003, FR-SUB-001, FR-ADS-001, FR-VIRAL-004]
effort_hours: 12
new_files:
...
allowed_tools:
  ...
  - "Daily reconciliation Sentry alerts (FR-OBS-002)"
disallowed_tools:
  ...
  - "Real-money convertibility from Coins (FR-LEGAL-002 anti-money-laundering posture)"
risk_if_skipped: "FR-CARE-001..004 spend Coins via stub; FR-PET-007 breeding cost stub; FR-ECON-002 IAP needs ledger; FR-ADS-001 rewarded video needs payout. Without ledger, the entire economy is invisible to anti-cheat (FR-OBS-002) and the launch economy will inflate uncontrollably."
...
- **Hearts** (premium) — purchasable via real-money IAP only (FR-ECON-002). Used for: premium species direct purchase, battle pass premium track unlock, cosmetic bundles. **Hearts MUST NOT randomly outcome anything** per FR-LEGAL-002.

§1.2  **Double-entry ledger.** Every currency mutation MUST be a paired row in `currency_ledger` table: one debit + one credit, summing to 0 net per transaction. Account types: `user_wallet`, `system_source` (rewards), `system_sink` (consumed in-game), `iap_source` (Hearts purchased), `creator_revshare` (FR-ECON-003 UGC).
...
§1.5  **Server-authoritative.** All ledger writes happen server-side. The Cocos client MUST NEVER write balance directly. Per FR-INFRA-002 §1.10.
...
§1.11  **Anti-cheat ban.** Per FR-INFRA-002 §1.9, any attempt to mutate balance via an invalid path (e.g. negative spend, cross-tenant move) → `security.impossible-transition` event + DPO review.
...
§1.13  **No Coins ↔ Hearts conversion.** Premium-to-soft conversion is forbidden — it would create a real-money-to-soft pathway (FR-LEGAL-002 randomisation concerns).

§1.14  **Free-tier daily cap on Coin earn.** Per FR-CARE-004 mini-game caps + FR-CARE-005 streak — cumulative Coin earn capped at 500/day for free tier, 1500/day for Pet+ subscribers. Anti-grind.

§1.15  **Hearts entry path.** Hearts arrive ONLY from FR-ECON-002 IAP. No earning path. No conversion.

§1.16  **Tenant scoping.** All ledger rows tagged `tenant_id`. Per FR-INFRA-003 §1.4. Cross-tenant ledger reads + writes forbidden.

§1.17  **Audit retention.** 7 years for kids-adjacent + 2 years for standard. Per FR-LEGAL-001.

§1.18  **DSR support.** On account deletion, ledger rows are tombstoned (not deleted) per FR-LEGAL-001 §3.6. Player can request export via `GET /v1/wallet/history?export=csv`.

§1.19  **Real-money compliance.** Hearts IAP receipts MUST be validated server-side via FR-ECON-002 receipt-validation. Untrusted client IAP claims are ALWAYS rejected.

§1.20  **Analytics.** `econ.ledger.entry { currency, amount, account_type, ref_type }`, `econ.balance.queried`, `econ.spend.rejected { reason }`, `econ.reconciliation.drift { drift_pct }` per FR-OBS-001.
...
**Why no Coins↔Hearts conversion.** Plan §PART 8 + FR-LEGAL-002 — real-money ↔ soft pathways trigger gambling-regulation scrutiny in BE/NL.
...
**Why ledger rows tombstoned on DSR.** GDPR Article 17 + FR-LEGAL-001 §3.6 — accounting evidence retained; PII redacted.
...
**Internal:** FR-INFRA-003 (Supabase + tenant), FR-INFRA-002 (anti-cheat path).
**Blocks:** FR-ECON-002 IAP (grants Hearts), FR-ECON-003 UGC rev-share, FR-SUB-001 (Pet+ subscription perks), FR-ADS-001 (rewarded video payout), FR-VIRAL-004 (battle pass).
...
| 9 | IAP receipt forgery → Hearts grant | FR-ECON-002 validation | Rejected | Verified via Apple/Google |
...
**Cross-reference:** FR-ECON-002 IAP grants Hearts; FR-CARE-001/002/004/005 + FR-PET-007 spend via ledger; FR-OBS-002 anti-cheat reads ledger.
