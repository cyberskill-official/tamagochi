...
created: 2026-05-17
shipped: 2026-05-17
related_frs: [FR-ECON-001, FR-ECON-002, FR-ART-001, FR-AI-002, FR-LEGAL-001, FR-LEGAL-002, FR-PET-001, FR-OBS-001, FR-AUTH-003]
depends_on: [FR-ECON-002, FR-AI-002]
blocks: []
effort_hours: 12
new_files:
...
allowed_tools:
  ...
  - "FR-AI-002 content safety on UGC name + description"
...
disallowed_tools:
  ...
  - "UGC replacing body or face slots (only accessory_head/accessory_body per FR-ART-001 §1.4)"
...
§1.1  **UGC scope.** Users design outfits for the `accessory_head` + `accessory_body` slots only (per FR-ART-001 §1.4 — body/face are off-limits). Each design has palette + simple Spine attachment shape.
...
§1.5  **Name + description content safety.** Submitter provides name (max 30 chars) + description (max 100 chars). Both pass through FR-AI-002 content safety + PII regex.
...
§1.7  **Purchase flow.** Buyer spends Coins (via FR-ECON-001 ledger). Coins split:
...
§1.13  **Kids SKU disabled.** Per FR-AUTH-003 — no submission, no purchase on kids SKU.
...
§1.15  **Approved UGC tenant scoping.** UGC submitted on consumer tenant (Mochi) is consumer-only. B2B tenants (FR-B2B-001) have their own UGC pool or none.
...
§1.20  **Analytics.** `ugc.submitted`, `ugc.reviewed { verdict }`, `ugc.purchased`, `ugc.creator.payout`, `ugc.featured` per FR-OBS-001.
...
**Internal:** FR-ECON-001 (ledger with creator_revshare account), FR-ECON-002 (Hearts grants), FR-AI-002 (content safety), FR-ART-001 (Spine slot system), FR-AUTH-003 (kids SKU).
**Blocks:** none.
...
**Cross-reference:** FR-AI-002 content safety, FR-ART-001 slot system, FR-ECON-001 ledger.
