...
- **ISS-002 (warning) Sentiment-anim map drift.** Round-1: animation names could drift from FR-ART-001 contract. **RESOLVED §10 row 12** — boot-time validation against contract names.
...
- **ISS-006 (warning) Kid mistakenly on standard SKU.** Round-2: SKU misdetection risk. **RESOLVED §10 row 11** — RLS + SKU gate via FR-AUTH-003.
...
- Sentiment→animation mapping integrates with FR-ART-001 contract.
- Kids SKU exclusion respects Snap/Replika regulatory precedent.

## §5 — Resolution

`score_post_revision_2: 10/10` — accept. FR-AI-001 is the unfair-advantage substrate per plan §PART 3 #3.
