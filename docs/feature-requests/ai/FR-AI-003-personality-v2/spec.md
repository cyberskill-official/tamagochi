...
created: 2026-05-17
shipped: 2026-05-17
related_frs: [FR-AI-001, FR-AI-002, FR-PET-005, FR-AUTH-003, FR-LEGAL-001, FR-OBS-001, FR-INFRA-003]
depends_on: [FR-AI-001, FR-AI-002, FR-PET-005]
blocks: []
effort_hours: 12
new_files:
...
allowed_tools:
  - "FR-AI-001 LLM router (Claude Haiku + Gemini Flash)"
  - "FR-AI-002 content safety"
  - "FR-PET-005 multi-pet roster"
disallowed_tools:
...
§1.1  **Standard SKU only.** Per FR-AI-001 §1.1 + FR-LEGAL-001 §1.5 — kids SKU stays scripted via FR-AI-002 dialogue trees. No LLM enrichment for under-13.

§1.2  **Selfie-aware reactions.** When player invokes FR-VIRAL-003 generative-pet flow OR explicitly shares a selfie via Settings, the AI may reference visual changes ("You got a haircut!"). Implementation:
- Player opt-in: settings toggle "Notice my appearance changes".
- On opt-in, periodic selfie comparison flow (optional, 1×/week).
- Selfie processed in-memory (NO storage per FR-LEGAL-001 §1.4).
- Dominant-color delta vs last reference → triggers "you look different!" line.
- Reference: 3-5 RGB values only persisted (no image).

§1.3  **Name-aware memory.** Player's name (display_name from FR-AUTH-001) included in persona prompt context. AI may reference: "Hi Linh!", "Linh, did you eat lunch?". DPO-reviewed for kid-appropriateness even on 13+.

§1.4  **Birthday awareness.** Player's birthday (from sign-up flow + FR-AUTH-001 — opt-in only) triggers "Happy birthday Linh!" line on the day.

§1.5  **Multi-pet sibling awareness.** Per FR-PET-005 multi-pet — AI persona context includes the player's roster. Siblings can reference each other: "Bao is so noisy today!" (when Bao is also owned). Tied to FR-PET-007 breeding lineage if siblings are bred.
...
§1.7  **Time-of-day awareness.** Per FR-AI-001 trigger `time_of_day` — v2 enriches: "Good morning Linh!" / "It's getting late, are you about to sleep?".

§1.8  **Event memory expansion.** Per FR-AI-001 §1.3 — increase 20-event memory to 40 for Pet+ subscribers (additional context for richer dialogue).
...
§1.10  **Content safety stricter for v2.** v2 responses pass through FR-AI-002 + additional name/birthday-leak check (don't echo back identifying info).
...
§1.13  **Name-leak prevention.** Per FR-AI-002 — AI MUST NOT include the player's name in shared TikTok clips (FR-VIRAL-001) or in any externally-rendered content. Only in in-app dialogue.
...
§1.17  **Tenant override.** B2B tenants may use baseline v1 only (FR-B2B-001 can disable v2 for tenant simplicity).

§1.18  **Performance budget.** v2 latency budget remains 2s P95 (same as v1 per FR-AI-001 §1.13).

§1.19  **Fallback to v1.** If v2 fails (cost cap, safety block, vendor outage), fall back to v1 (scripted) per FR-AI-001 fallback chain.

§1.20  **Analytics.** `ai.persona_v2.feature_used { feature }`, `ai.persona_v2.name_referenced`, `ai.persona_v2.sibling_referenced`, `ai.persona_v2.selfie_delta_detected` per FR-OBS-001.
...
**Why deterministic anti-affinity.** Plan §PART 4 + FR-LEGAL-002 — no random outcomes; auditable.
...
export class PersonaV2Service {
  async speak(petId: string, ownerId: string, trigger: PersonaTrigger, overrides?: PersonaContextOverrides) {
    ...
    try {
      ...
      // Name-leak prevention for shareable content (per FR-VIRAL-001)
...
**External:** Same as FR-AI-001 (Claude Haiku, Gemini Flash).
**Internal:** FR-AI-001 (LLM router), FR-AI-002 (safety), FR-PET-005 (multi-pet), FR-AUTH-003 (kids exclusion).
**Blocks:** none.
...
**Cross-reference:** FR-AI-001 fallback; FR-PET-005 multi-pet roster; FR-VIRAL-001 share redaction.
