...
created: 2026-05-17
shipped: 2026-05-17
related_frs: [FR-PET-001, FR-PET-002, FR-PET-003, FR-PET-005, FR-CARE-005, FR-LEGAL-002, FR-INFRA-003, FR-OBS-001, FR-AI-001, FR-VIRAL-005]
depends_on: [FR-PET-003, FR-LEGAL-002]
blocks: []
effort_hours: 10
new_files:
...
allowed_tools:
  ...
  - "FR-VIRAL-005 push for rescue-available notification"
disallowed_tools:
...
risk_if_skipped: "Plan §PART 3 + FR-LEGAL-002 §1.5 — Permadeath-Lite is the ethical alternative to hard Tamagotchi permadeath. Without it, the neglect signal (FR-PET-003 §1.8 critical-stat warnings) has no terminal consequence, weakening engagement loop."
...
§1.2  **Pre-grandma warnings.** At day 3 of full-stat-zero neglect, player receives push: "Mochi is feeling really lonely. Come say hi soon?" (FR-VIRAL-005 sleep-hour respected). At day 5: "Mochi misses you a lot. We're worried." At day 7: pet enters grandma_house + notification "Mochi has gone to stay with grandma. You can bring them home anytime."

§1.3  **Grandma state.** When `pets.stage = 'grandma_house'`:
- Pet does NOT decay (FR-PET-003 §1.2).
- Pet is NOT visible in main game scene.
- Pet appears in a special "Grandma's House" UI section.
- Pet does NOT count against slot quota (FR-PET-005 §1.2).
- AI persona (FR-AI-001) returns a "missing you" line.
...
§1.6  **Rescue completion.** After 3 ritual days, pet returns to `pets.stage = <prior stage>` (preserved from `pets.last_active_stage` column added in this FR). All stats reset to 50 (mid-range — not punitive, not full).

§1.7  **No real-money revival.** Per FR-LEGAL-002 §1.5 + Belgium 2018 + Antwerp 2025 — no IAP path to bypass the 3-day ritual. The ritual is the only path.
...
§1.10  **Rescue grandfathering.** A pet that has been to grandma_house and back retains its species + tier + palette_seed. FR-PET-006 §1.10 — no tier downgrade.

§1.11  **Rescue audit.** `grandma_rescues` table: `(pet_id, owner_id, entered_grandma_at, ritual_days, completed_at, source, tenant_id)`. Per FR-LEGAL-001 audit retention.
...
§1.16  **AI persona during grandma.** Per FR-AI-001 §1.5, persona prompt context includes "this pet is at grandma's house" — LLM responds accordingly ("I miss you..."). Plan §PART 3 + ethical retention.

§1.17  **No real-money skip cooldown.** The 18-hour cooldown cannot be skipped via IAP. Plan §PART 8.

§1.18  **Trade/breeding eligibility.** Grandma_house pets CANNOT be traded (FR-SOCIAL-003) or bred (FR-PET-007). Must rescue first.

§1.19  **Tenant-scoping.** Cross-tenant grandma transitions impossible per FR-INFRA-003 §1.4.

§1.20  **Analytics.** `pet.grandma.entered`, `pet.grandma.warned { day }`, `pet.grandma.rescue.initiated`, `pet.grandma.rescue.ritual_day { day }`, `pet.grandma.rescued`, `pet.grandma.rescue.expired { ritual_days_completed }` per FR-OBS-001.
...
**Why escalating warnings.** Plan §PART 5 + FR-CARE-005 forgiveness — give the player multiple chances. 3-day warning + 5-day + 7-day forms a clear narrative.
...
**Why grandma doesn't count against slot.** FR-PET-005 §1.2 — encourages rescue over abandonment. Punishing the slot count would push players to give up.

**Why grandma persists species + tier.** FR-PET-006 §1.10 — tier identity matters. Tier downgrade would feel like punishment.

**Why no real-money skip.** Plan §PART 8 + FR-LEGAL-002 — Belgium/NL/EU loot-box-adjacent rules + HoYoverse precedent.
...
```typescript
// apps/api/src/pets/permadeath/__tests__/grandma-watch.spec.ts
describe('FR-PET-008 — grandma watch', () => {
...
**Internal:** FR-PET-003 (stat values), FR-LEGAL-002 (no-real-money policy), FR-VIRAL-005 (push notification for warnings), FR-AI-001 (grandma persona line).
**Blocks:** none directly (terminal state); FR-VIRAL-002 may reference grandma events.
...
| 3 | Push notifications during sleep hours | FR-VIRAL-005 catches | Suppressed | Already covered |
...
| 7 | Real-money rescue accidentally allowed via IAP | FR-LEGAL-002 lint | Build blocked | Lint catches |
...
**Cross-reference:** FR-PET-005 (grandma excluded from slot count); FR-PET-006 (tier preserved); FR-SOCIAL-003 (trade blocked); FR-PET-007 (breed blocked); FR-AI-001 (grandma persona).
