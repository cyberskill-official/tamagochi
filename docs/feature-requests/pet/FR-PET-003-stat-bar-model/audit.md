...
- **ISS-004 (error) RLS on `pet_stat_history` not enforced.** Round-2 noted §1.18 mentioned co-parent visibility but the migration in §3.3 did not have an RLS policy that scoped to co-parent. **RESOLVED §3.3 + AC11** — RLS uses sub-select against `pets.owner_id = auth.uid()`; co-parent extension via FR-SOCIAL-002 will widen.
...
`score_post_revision_2: 10/10` — accept. FR-PET-003 is the care-loop signal substrate. FR-CARE-001/002/003/004/005 + FR-PET-008 all depend on it.
