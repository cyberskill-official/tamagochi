...
created: 2026-05-17
shipped: 2026-05-17
related_frs: [FR-PET-001, FR-PET-003, FR-INFRA-002, FR-ART-001, FR-CARE-001, FR-CARE-002, FR-CARE-003, FR-CARE-005, FR-PET-007, FR-PET-008, FR-AI-001, FR-VIRAL-001, FR-OBS-002]
depends_on: [FR-PET-001, FR-ART-001]
blocks: [FR-PET-003, FR-PET-007, FR-PET-008, FR-CARE-005, FR-VIRAL-001]
effort_hours: 8
new_files:
...
allowed_tools:
  ...
  - "Cocos `sp.Skeleton` skin swap (FR-ART-001 §1.3)"
disallowed_tools:
  ...
  - "`Math.random()` for stage-gated outcomes — use crypto RNG per FR-LEGAL-002"
risk_if_skipped: "Without server-authoritative age + evolution, the most-asked-for cheat in Tamagotchi-style games (skip time to evolve) becomes trivial; FR-PET-007 (breeding) requires `stage='adult'` gate, so breeding economy collapses if evolution is spoofable."
audience_age_gate: "any"
---

## §1 — Description (BCP-14 normative)

§1.1  **Stage definitions.** A pet's lifecycle MUST progress through five stages: `egg → baby → teen → adult → grandma_house`. `grandma_house` is the Permadeath-Lite state (FR-PET-008) — neglect, not natural progression. The natural arc is `egg → baby → teen → adult`, irreversible.

§1.2  **Age durations (real-time hours).** Each natural stage MUST have a configurable duration in real-time hours:
- `egg → baby`: 0.5 h (the hatch ceremony — short, completed via FR-PET-001).
- `baby → teen`: 48 h (≈ 2 days).
- `teen → adult`: 168 h (≈ 7 days).

Durations are read from `apps/api/src/pets/evolution/evolution-config.ts`. A/B tunable per FR-OBS-001 feature flags (e.g. `evolution.baby_to_teen.duration_h` overrides default for a cohort).

§1.3  **Wall-clock-resistant age timer.** Pet age MUST be computed server-side from the **monotonic UTC timestamp** of `pets.born_at` (FR-PET-001 §1.1). The client never sends "minutes since X" — it asks `GET /v1/pets/:id` which returns `age_minutes` computed at request time. Client-side rendering displays the server's `age_minutes` only.
...
- `egg`: only `hatch_animation` + naming (FR-PET-001).
- `baby`: all care actions (FR-CARE-001/002/003), 1 mini-game (FR-CARE-004), no AR (FR-AR-001 disabled), no co-parent invite (FR-SOCIAL-002 disabled).
- `teen`: AR placement enabled, co-parent invite enabled, mini-games full set.
- `adult`: breeding eligibility (FR-PET-007), trade eligibility (FR-SOCIAL-003), wedding-ceremony eligibility (FR-SOCIAL-004), generative-pet reproduction (FR-VIRAL-003).
- `grandma_house`: NO actions except the daily-ritual rescue (FR-PET-008).

Gate enforcement MUST live in `stage-gates.ts` as a single source of truth queried by every action handler.

§1.7  **Spine skin swap on advancement.** Per FR-ART-001 §1.3, the skeleton has three skin presets (`baby`, `teen`, `adult`). On stage advance, the Cocos `EvolutionFlow.ts` MUST: (a) play the `evolve_baby_to_teen` (or `evolve_teen_to_adult`) contract animation; (b) at the animation's midpoint, swap the Spine skin; (c) play 1 s of confetti particles + `success` haptic; (d) emit `pet.evolution.viewed` analytics.

§1.8  **TikTok-native share moment.** A stage advance is a viral moment — `EvolutionFlow.ts` MUST surface a "Share this milestone?" prompt to the player after the celebration, routing to FR-VIRAL-001's TikTok export pipeline. The prompt is opt-in; on the kids SKU, it's disabled outright (no out-of-app sharing for under-13 per FR-LEGAL-003 §1.4).
...
§1.10  **Skip-time anti-cheat.** Per FR-OBS-002 (P4) but with FR-PET-002's groundwork, any attempt to manipulate `born_at` (e.g. via direct DB write through a leaked service-role key) MUST be detected by a daily reconciliation job that compares `born_at` against the `pet_age_events` audit table: a `born_at` row earlier than the first `pet.hatched` audit event is suspicious + alerts the DPO.
...
§1.13  **Sleep-hour respect for advancement notifications.** On stage advance, push notification (FR-VIRAL-005) is suppressed during the player's local 22:00–07:00 window (or 20:00–08:00 for under-13). The advancement still happens; the notification is delivered the following morning.

§1.14  **Cocos `EvolutionFlow.ts` UI.** The Cocos component MUST: (a) subscribe to `PetState.stage` changes; (b) on change to a new stage, play the FR-ART-001 contract animation; (c) handle the swap-skin moment cleanly without flicker; (d) preserve the camera framing across the swap; (e) emit `pet.evolution.viewed` even if the player misses the animation (came back later).
...
§1.17  **No retrograde stages.** Once a pet reaches `adult`, it MUST NOT return to `teen` or `baby`. The `grandma_house` state is the only "downward" transition + is explicitly Permadeath-Lite (FR-PET-008) not natural ageing.
...
§1.19  **Feature-flag tunability.** Stage durations + care-gate thresholds MUST be readable via Mixpanel feature flags (per FR-OBS-001 §1.10) so live-ops can tune live without redeploy. Defaults baked into code so the system works even if Mixpanel is down (FR-OBS-001 §10 row 3).

§1.20  **Analytics events.** `pet.stage.advance`, `pet.stage.stall { reason }`, `pet.evolution.viewed`, `pet.evolution.shared` MUST be emitted per FR-OBS-001 §1.4 taxonomy. Schema-validated per FR-OBS-001 §1.5.
...
**Why kids SKU disables share.** FR-LEGAL-003 §1.4 — out-of-app links require parental gate. Stage-share to TikTok is an out-of-app action. Disabling on kids SKU is simpler than gating; the kids version saves the milestone locally for the parent to see in the parental dashboard (FR-SUB-002).
...
**Internal:** FR-PET-001 (pets table + born_at column); FR-ART-001 (Spine contract animations `evolve_baby_to_teen`, `evolve_teen_to_adult` + skin presets); FR-INFRA-002 (Colyseus tick loop + room broadcast).

**Blocks:** FR-PET-003 (stat-bar decay uses stage to set decay rates); FR-PET-007 (breeding requires `stage='adult'`); FR-PET-008 (Permadeath-Lite reads stage); FR-CARE-005 (streak system); FR-VIRAL-001 (share-on-evolution).
...
- `Math.random()` for evolution outcomes (although none currently random; rule applied prospectively for FR-PET-007 breeding traits).

**Cross-reference:** This FR is the pet-lifecycle substrate. FR-PET-003 (stat decay rates by stage), FR-PET-007 (breeding adult gate), FR-PET-008 (Permadeath-Lite from any active stage) all depend on stage being trustworthy.
