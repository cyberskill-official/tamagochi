...
created: 2026-05-17
shipped: 2026-05-17
related_frs: [TASK-INFRA-001, TASK-INFRA-002, TASK-INFRA-003, TASK-AUTH-002, TASK-AUTH-003, TASK-LEGAL-001, TASK-LEGAL-003, TASK-OBS-001, TASK-SUB-001, TASK-SUB-002]
depends_on: [TASK-INFRA-003]
blocks: [TASK-AUTH-002, TASK-AUTH-003, TASK-SUB-001, TASK-B2B-003]
effort_hours: 10
new_files:
...
disallowed_tools:
  ...
  - "Embedding service_role keys client-side (re: TASK-INFRA-003 §1.6)"
risk_if_skipped: "Without a working multi-provider sign-in, no task that requires player identity can be delivered; the kids SKU specifically cannot ship without parental consent ↔ child account binding, which requires this task's `region_of_record` derivation."
...
§1.15  **Apple Sign-In privacy compliance.** Apple Sign-In is REQUIRED by Apple App Store Review if any third-party social sign-in is offered. The standard SKU offers Google + Apple; the kids SKU offers neither directly (parental consent flow per TASK-AUTH-003 is the only entry).
...
**Why disable email/password.** Passwordless authentication is the modern UX standard for consumer mobile apps. Email/password adds breach-blast-radius (every leak is a credential leak), reset flows, password-hint UX, and CAPTCHA pressure. Apple Sign-In + Google + Zalo (TASK-AUTH-002) cover ~99% of the target VN/global audience.
...
**Why no SKU commingling.** Per TASK-LEGAL-001 + TASK-INFRA-003 §1.1, two databases means a user's identity is partitioned by SKU. The explicit refusal message + graduation flow prevents silent data spread.
...
**Why multi-device sessions are concurrent.** PetPair (TASK-SOCIAL-002) explicitly requires co-parent multi-device. Forcing single-device login would block PetPair. Colyseus presence + room reconciliation handles consistency.
...
describe('TASK-AUTH-001 §1.11 — proactive token refresh', () => {
...
describe('TASK-AUTH-001 §1.6 — region derivation', () => {
...
**Internal:** TASK-INFRA-003 (Supabase + RLS templates).

**Blocks:** TASK-AUTH-002 (Zalo follows the same shape), TASK-AUTH-003 (kid invite-code uses the consent flow which is keyed off this task's account creation), TASK-SUB-001 (Pet+ subscription is tied to the user id created here), TASK-B2B-003 (PetOS console SSO bridge piggybacks on this scaffold).
...
|---|---|---|---|---|
| 1 | Apple Sign-In service outage | `auth.signin.failure { provider: apple }` spike | iOS users blocked | Surface fallback to Google sign-in; in VN, fallback to Zalo (TASK-AUTH-002) |
...
**Cross-reference:** This task is the consumer-SKU authentication keystone. TASK-AUTH-002 (Zalo) follows the same shape with Zalo-specific quirks. TASK-AUTH-003 (kid invite-code) layers parental consent on top.
