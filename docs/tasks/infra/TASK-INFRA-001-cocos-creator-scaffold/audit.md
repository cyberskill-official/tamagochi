...
- **ISS-001 (warning) Tenant slug default ambiguous.** Round-1 noted `tenantSlug?: string` could resolve to `undefined` in `loadBundle`, leading to `https://cdn/undefined/bundle`. **RESOLVED §3.2** — default `slug = opts.tenantSlug ?? 'mochi'` (consumer tenant slug); the §11 cross-reference to TASK-B2B-001 documents the slug taxonomy.
...
`score_post_revision_2: 10/10` — accept. This task is the substrate for every visual / gameplay task (TASK-ART-001 → TASK-PET-001 → all of P1). The scaffold is now reproducible, audit-friendly, and pre-emptively COPPA-safe.
