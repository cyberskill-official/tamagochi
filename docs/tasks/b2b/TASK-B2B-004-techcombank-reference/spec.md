...
created: 2026-05-17
shipped: 2026-05-17
related_frs: [TASK-B2B-001, TASK-B2B-002, TASK-B2B-003, TASK-CARE-004, TASK-LEGAL-001, TASK-AUTH-003, TASK-AUTH-001, TASK-ECON-001, TASK-OBS-001]
depends_on: [TASK-B2B-003, TASK-CARE-004]
blocks: []
effort_hours: 14
new_files:
...
allowed_tools:
  ...
  - "Per-tenant mini-game framework (TASK-CARE-004)"
  - "TASK-B2B-001 manifest schema"
disallowed_tools:
...
§1.1  **Tenant slug `techcombank`.** Per TASK-B2B-001 — slug fixed, manifest stored at `apps/cocos/assets/tenants/techcombank/manifest.json`.

§1.2  **Audience.** Junior banking account holders — under-13 audience by default. Kids SKU strict per TASK-AUTH-003 + TASK-LEGAL-001.

§1.3  **Bundle ID.** `world.techcombank.junior-pet` (Apple) / `vn.com.techcombank.juniorpet` (Android).

§1.4  **Techcombank SSO bridge.** Techcombank account holders sign in via Techcombank's banking app SSO (OAuth 2.0 + PKCE). Per TASK-AUTH-001 — Techcombank tenant uses Techcombank IdP + parental consent path via TASK-AUTH-003.

§1.5  **Pet branded as Techcombank Junior pet.** Custom mascot (still respecting TASK-ART-001 §1.2 20-animation contract), Techcombank palette + logo overlay.

§1.6  **Savings-quest mini-game.** A new mini-game extending TASK-CARE-004 framework: "Save 100K VND this week" — the pet "saves" virtual money along with the child's actual savings account activity (Techcombank API webhook).
...
§1.8  **Financial-literacy mini-game.** Quiz mini-game: "What does 'savings' mean?" — fun + educational. Per TASK-CARE-004 — added as `financial-literacy-quiz` game.

§1.9  **Per-account-holder unique mascot.** Each child gets a unique pet (TASK-PET-001 ULID); palette + tier seeded by `(child_id, parental_consent_id)` deterministic hash.

§1.10  **No real-money out from pet.** Per TASK-LEGAL-002 — pet rewards never convert to real money or affect bank account balance. One-way: bank → pet (rewards) only.
...
§1.12  **No trade with consumer.** Per TASK-B2B-002 + plan §PART 6 — Techcombank tenant pets cannot be traded to consumer tenant.
...
§1.16  **Parental consent integration.** Techcombank junior account holders are by definition under-18 + parental-account-linked. TASK-AUTH-003 consent flow uses Techcombank-provided parent contact.

§1.17  **Audit retention — Techcombank 10 years.** Banking-grade retention (longer than 7-year COPPA default).

§1.18  **Quest CMS managed by Techcombank.** Per TASK-B2B-003 — Techcombank operator account configures quest catalogue.

§1.19  **DPO review of all Techcombank quest copy + AI dialogue if any.** Per TASK-LEGAL-001.

§1.20  **Analytics — Techcombank workspace.** Per TASK-OBS-001 §1.15 — separate Mixpanel + GameAnalytics.
...
```typescript
describe('TASK-B2B-004 — Techcombank', () => {
...
export class SavingsQuestLogic implements MiniGameContract {
  ...
  async startWithSeed(_seed: string, _opts: any): Promise<void> {
    ...
    // Linked to real-time data via TASK-B2B-001 manifest CMS strings.
  }
...
**Cross-reference:** TASK-B2B-001 manifest; TASK-B2B-002 partition; TASK-CARE-004 mini-game extension; TASK-AUTH-003 consent.
