...
created: 2026-05-17
shipped: 2026-05-17
related_frs: [TASK-INFRA-001, TASK-ART-001, TASK-I18N-001, TASK-PET-006, TASK-B2B-002, TASK-B2B-003, TASK-B2B-004, TASK-B2B-005, TASK-OBS-001]
depends_on: [TASK-INFRA-001, TASK-ART-001, TASK-I18N-001]
blocks: [TASK-B2B-002, TASK-B2B-003, TASK-B2B-004, TASK-B2B-005]
effort_hours: 14
new_files:
...
disallowed_tools:
  - "Tenant overrides that remove core animations (TASK-ART-001 §1.2 contract)"
  - "Tenant cross-data sharing (TASK-B2B-002 partition enforced)"
...
§1.3  **Theme bundle.** Tenant theme assets (palette + logo + Spine skin + per-species mascots) lazy-loaded from Cloudflare R2 by slug. Per TASK-INFRA-001 §1.5 + 1.6 bundle budget — tenant override ≤ 4 MB.

§1.4  **Quest CMS.** Per-tenant quest strings + flow definitions loaded from CMS API. Tenant operators define quests via the PetOS console (TASK-B2B-003).
...
§1.6  **No contract violation.** Tenant overrides MAY change palette + accessories + names but MUST NOT remove TASK-ART-001 §1.2 20-animation contract.
...
§1.9  **Tenant rate-limit budget.** Per TASK-INFRA-002 §1.15 — each tenant has its own rate-limit budget. Consumer 'mochi' separate from B2B.

§1.10  **Locale + tenant interaction.** Tenant locale overrides TASK-I18N-001 base strings via override JSON files. Per `i18n/tenants/<slug>/<locale>/*.json`.
...
§1.15  **Per-tenant analytics workspace.** Per TASK-OBS-001 §1.15 — each tenant has its own GameAnalytics + Mixpanel workspace.
...
§1.20  **Analytics.** `tenant.bootstrap.success`, `tenant.theme_loaded`, `tenant.feature_flag.toggled`, `tenant.signature.invalid` per TASK-OBS-001.
...
**Why contract preserved.** TASK-ART-001 §1.2 — all gameplay code assumes the 20 animations exist.
...
```typescript
describe('TASK-B2B-001 — multi-tenant', () => {
...
**Cross-reference:** TASK-B2B-002 (partition + RLS); TASK-B2B-003 (PetOS console).
