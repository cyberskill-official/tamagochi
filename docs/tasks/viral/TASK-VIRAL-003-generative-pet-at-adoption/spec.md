...
created: 2026-05-17
shipped: 2026-05-17
related_frs: [TASK-PET-001, TASK-PET-006, TASK-AI-001, TASK-AI-002, TASK-ART-001, TASK-AUTH-003, TASK-LEGAL-001, TASK-LEGAL-002, TASK-VIRAL-001, TASK-OBS-001]
depends_on: [TASK-PET-006, TASK-AI-002, TASK-ART-001]
blocks: []
effort_hours: 12
new_files:
...
allowed_tools:
  ...
  - "OpenAI Moderation API (TASK-AI-002) for prompt safety"
  - "Optional vision API (Google Vision or AWS Rekognition) for selfie safety classification"
disallowed_tools:
  - "Generative pet on kids SKU (TASK-AUTH-003 — kid pets are scripted-derived)"
...
§1.1  **Generative pet on adoption.** Only at the moment of first hatching a NEW pet (per TASK-PET-001 §1.4), 13+ players have an optional path to provide a text prompt OR selfie to influence the pet's appearance.

§1.2  **Standard SKU only.** Per TASK-AUTH-003 + plan §PART 8, kids SKU pets use the default palette derivation per TASK-PET-001 §1.6 (server-seeded). Generative path is disabled.
...
§1.5  **Prompt safety.** Text prompt MUST pass TASK-AI-002 content safety. Selfie MUST pass an OPTIONAL vision-API safety classifier (faces detected → flag for review but not blocked; NSFW → blocked).
...
§1.7  **One-of-one uniqueness.** Combined with TASK-PET-001 §1.6 server-side `palette_seed`, the result is a unique 16-byte palette. Two players with identical prompts get slightly different results due to the seed.

§1.8  **Spine palette swap.** Cocos `SpineLoader` (TASK-ART-001 §1.4) applies the palette via Spine's slot-recolour mechanism. The 4 attachment slots (`body`, `face`, `accessory_head`, `accessory_body`) MUST respect the palette.

§1.9  **Endpoint — adopt with prompt.** `POST /v1/pets/hatch` (existing TASK-PET-001 endpoint, extended) — body adds optional `{ generative_input: { mode: 'text'|'selfie', prompt?: string, dominant_colors?: string[] } }`.

§1.10  **Cost.** Free at P2 (encourage adoption). TASK-ECON-001 may extend with cosmetic re-generation options in P3+.

§1.11  **One generative input per pet.** Once hatched, the palette is locked. Re-roll is not available; player must buy a "Pet Couture" cosmetic to change appearance (TASK-ECON-003).
...
§1.13  **Prompt sanitisation.** Text prompt is stripped of any PII regex patterns (email/phone/URL) before being passed to TASK-AI-002 — defense in depth.
...
§1.16  **Cocos `GenerativePetAdoptionUi.ts`.** UI offers 3 paths: "Surprise me" (default palette per TASK-PET-001), "Describe a feeling" (text prompt), "Use a photo" (selfie). 90-second budget per onboarding (TASK-PET-004).

§1.17  **Tenant override blocks generative.** B2B tenants (TASK-B2B-001) may disable the generative path — their themes are pre-curated.

§1.18  **Locale.** Prompt input + UX copy localised per TASK-I18N-001.

§1.19  **AR compatibility.** Generative palette renders correctly in AR (TASK-AR-001) — palette is per-skeleton, not per-scene.

§1.20  **Sharing the unique pet.** First-hatch share via TASK-VIRAL-001 includes "Generated from: <prompt or 'a photo'>" caption attribution (text-mode only — selfie mode never reveals).
...
**Why kids SKU disabled.** Plan §PART 8 + TASK-AI-002 — kid generative is regulator-toxic.
...
export function generatePalette(input: {
  species: Species;
  baseSeed: Buffer;        // TASK-PET-001 server-side
...
```typescript
describe('TASK-VIRAL-003 — generative palette', () => {
...
**Cross-reference:** TASK-PET-006 species + tier carry forward; TASK-VIRAL-001 share attribution.
