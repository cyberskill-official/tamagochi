...
created: 2026-05-17
shipped: 2026-05-17
related_frs: [TASK-CARE-001, TASK-CARE-002, TASK-CARE-003, TASK-CARE-004, TASK-PET-003, TASK-OBS-001, TASK-INFRA-003, TASK-VIRAL-004, TASK-VIRAL-005, TASK-LEGAL-001]
depends_on: [TASK-CARE-001, TASK-CARE-002, TASK-CARE-003, TASK-CARE-004]
blocks: [TASK-VIRAL-004, TASK-VIRAL-005]
effort_hours: 8
new_files:
...
disallowed_tools:
  ...
  - "Real-money forgiveness tokens (must be earned only per TASK-LEGAL-002)"
  - "Cozy Hour bonuses gated by sub tier (Pet+ gets +1 bonus token monthly per TASK-SUB-001 — that's the only gating)"
...
§1.2  **Day boundary.** A "day" boundary is the player's local-region midnight (region from TASK-AUTH-001 §1.6 deterministic chain). Activity between local 00:00 and 23:59 counts for that day.
...
§1.6  **Pet+ bonus.** Pet+ subscribers (TASK-SUB-001) get +1 bonus forgiveness token monthly (4 total). The bonus is the ONLY sub-tier-gated streak feature.

§1.7  **Forgiveness token notification.** When a token is auto-applied, the player receives:
- Push notification: "We covered your streak yesterday — see you today!" (sleep-hour respected per TASK-VIRAL-005).
...
- All care actions yield double Coins (TASK-ECON-001 ledger, P3 — at P1 the multiplier is stubbed).
- Stat decay halves (TASK-PET-003 §1.3 modifier flag).
...
§1.14  **Streak recompute on activity.** Every successful care action (TASK-CARE-001/002/003/004) MUST trigger a streak recompute via `StreakService.recordActivity(userId)`. The service idempotently advances the streak if it's the player's first activity of the day, else no-op.
...
Milestone rewards persist to TASK-ECON-001 ledger (stubbed at P1).

§1.17  **No streak leaderboard on kids SKU.** Per TASK-LEGAL-003 + plan §PART 8, no social comparison on kids accounts. Standard SKU may show per-player history but not vs-friends comparison.
...
§1.19  **Audit + analytics.** `streak.day.recorded`, `streak.token.applied`, `streak.reset`, `streak.milestone.reached`, `cozy_hour.entered`, `cozy_hour.exited` events per TASK-OBS-001 §1.4.
...
**Why Pet+ gets +1 token only.** TASK-SUB-001 wants meaningful perks but not pay-to-streak. +1 token is a small lubricant.
...
- **OQ-4 (resolved):** Forgiveness token earned via gameplay? → §1.16 — milestone rewards include bonus tokens; TASK-LEGAL-002 keeps it earned-only.
...
| 4 | Push notification sent during sleep hours | Per TASK-VIRAL-005 catch | Suppressed | Already covered |
...
| 7 | Pet+ subscriber starts month with 3 not 4 tokens | TASK-SUB-001 entitlement bug | Customer complaint | Manual +1 + investigate entitlement |
...
**Cross-reference:** TASK-VIRAL-004 battle pass references streak; TASK-VIRAL-005 push frequency reads streak state; TASK-SUB-001 +1 token entitlement.
