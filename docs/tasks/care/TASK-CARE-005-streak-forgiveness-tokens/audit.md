...
- **ISS-006 (info) Pet+ +1 token entitlement bug.** Round-2: TASK-SUB-001 integration risk. **RESOLVED §10 row 7** — manual +1 + investigate path.
...
- Cross-references to TASK-VIRAL-004/005, TASK-SUB-001, TASK-LEGAL-001 are explicit.

## §5 — Resolution

`score_post_revision_2: 10/10` — accept. CARE slice complete (5/5). TASK-PET-002 care-gated stall + this streak combo is the canonical daily-engagement substrate.
