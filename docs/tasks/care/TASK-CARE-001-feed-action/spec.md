...
created: 2026-05-17
shipped: 2026-05-17
related_frs: [TASK-PET-001, TASK-PET-002, TASK-PET-003, TASK-ART-001, TASK-INFRA-002, TASK-INFRA-003, TASK-CARE-005, TASK-ECON-001, TASK-OBS-001, TASK-LEGAL-002, TASK-VIRAL-002, TASK-B2B-005]
depends_on: [TASK-PET-003, TASK-ART-001]
blocks: [TASK-CARE-005, TASK-VIRAL-002, TASK-B2B-005]
effort_hours: 8
new_files:
...
allowed_tools:
  - "Server-authoritative care handler (TASK-INFRA-002 §1.10)"
  - "Postgres care_actions audit table"
  - "Spine `eat` animation (TASK-ART-001 contract)"
  - "HapticsAdapter.tap('light') on feed"
disallowed_tools:
  - "Client-driven stat mutation"
  - "Real-money food (food is earned currency only per TASK-LEGAL-002)"
  - "Feeding pets in `egg` or `grandma_house` stages"
risk_if_skipped: "Without server-authoritative feed, the core care loop has no anti-cheat foundation; hunger is the highest-decay stat (TASK-PET-003 §1.2) so its mutation path is the most-attacked surface; TASK-CARE-005 streak system reads care-action audit log."
...
§1.2  **Stage gate.** Per TASK-PET-002 §1.6, feeding MUST be permitted only when `stage ∈ {baby, teen, adult}`. Eggs and grandma_house pets reject with HTTP 403 `{ error: 'pet.stage.locked', current_stage }`.

§1.3  **Ownership check.** Only the pet's `owner_id` OR an `active` co-parent (TASK-SOCIAL-002 P2 wiring) MAY feed. Cross-tenant + non-owner attempts return HTTP 403.
...
Catalogue in `apps/api/src/care/food-catalogue.ts` is a single source of truth; TASK-ECON-002 (P3) extends with cosmetic-tied food but the schema is locked here.

§1.5  **Currency spend.** Server MUST deduct Coins via TASK-ECON-001 (P3) ledger. At P1, the ledger is stubbed to "always succeed" with infinite Coins — TASK-ECON-001 wires the real ledger. The contract MUST allow rejection with `{ error: 'insufficient_coins', required, have }` for forward-compatibility.
...
§1.7  **Stat apply.** Apply via `StatDecayService.apply(stats, 'feed_basic' | 'feed_premium')` per TASK-PET-003 §3.1. Result clamped 0-100.

§1.8  **`last_seen_at` update.** Each feed MUST update `pets.last_seen_at` so decay (TASK-PET-003) restarts from now.
...
§1.15  **Analytics.** Emit `care.feed.success { food_id, hunger_before, hunger_after, source }` + `care.feed.failure { reason }` per TASK-OBS-001 §1.4.

§1.16  **Impossible-state detection.** Feeding when hunger=100 triggers TASK-INFRA-002 §1.9 impossible-transition path; this task's handler MUST call `recordImpossibleTransition` and reject with HTTP 422 `{ error: 'pet.stat.already_full' }`.

§1.17  **Source attribution.** The `source` field distinguishes `ui` (the owner-tapped feed) from `co_parent_remote` (the co-parent fed in TASK-SOCIAL-002). This is the substrate for the PetPair receipt notification ("Linh fed Mochi while you were asleep").

§1.18  **No real-money food.** Per TASK-LEGAL-002 §1.2, food is earned-currency only. The catalogue MUST NOT include any `iap_sku` field; TASK-ECON-002's IAP catalogue cannot reference food items.
...
**Why server-authoritative.** Plan §PART 4 anti-cheat. Feed is the single most common care action; if client-spoofable, the entire stat economy collapses + breeding eligibility (TASK-PET-007) becomes free.

**Why two foods at P1.** Plan §PART 3 economy basics. Two tiers give a meaningful currency-to-effectiveness gradient without overwhelming the player; TASK-ECON-002 expands the catalogue in P3.
...
**Why broadcast over PetRoom.** Co-parent presence (TASK-SOCIAL-002) needs to see live state. Broadcasting the action explicitly (rather than relying solely on schema diff) gives a notification surface ("Linh fed Mochi!") + an analytics seam.
...
**AC6.** Insufficient coins (when TASK-ECON-001 wires real ledger) returns 402 `insufficient_coins`. Verified by spec test with stub ledger throwing.
...
describe('TASK-CARE-001 — feed action', () => {
...
**Internal:** TASK-PET-001 (pets table), TASK-PET-002 (stage gate), TASK-PET-003 (StatDecayService.apply), TASK-ART-001 (`eat` animation), TASK-INFRA-002 (Colyseus broadcast), TASK-INFRA-003 (care_actions table + RLS).

**Blocks:** TASK-CARE-005 (streak system reads care_actions log), TASK-VIRAL-002 (Daily Drama event types include feed), TASK-B2B-005 (Viettel tenant feed-with-topup uses this contract).
...
- **OQ-1 (resolved):** Two foods vs more? → §1.4 — two at P1; TASK-ECON-002 expands.
...
| 3 | Currency ledger stub returns success when real ledger would reject | E2E test in TASK-ECON-001 | Pet over-fed for free | Acceptable at P1; tightens when ledger lands |
| 4 | Concurrent feed (owner + co-parent simultaneously) | Race on `pets` update | One feed lost | Per-pet mutex in PetRoom serialises (TASK-PET-003 §10 row 12) |
| 5 | Cooldown disclaimer + UX hint missing | Player support tickets | Frustration | Surface `retry_after_sec` to UI countdown |
| 6 | Premium food shipped without IAP path expectation | Audit | Soft-launch confusion | TASK-ECON-001 ledger clarifies — food is Coin-only |
...
| 12 | Rate-limit triggers on legitimate Pet+ owner with 10 pets | Support ticket | Frustration | Raise to 120/hour for Pet+ via TASK-SUB-001 entitlement |
...
**Cross-reference:** TASK-CARE-002/003/004 follow this pattern. TASK-CARE-005 reads `care_actions` for streak detection.
