...
created: 2026-05-17
shipped: 2026-05-17
related_frs: [TASK-ADS-001, TASK-AUTH-003, TASK-LEGAL-001, TASK-LEGAL-003, TASK-ECON-001, TASK-OBS-001, TASK-INFRA-003]
depends_on: [TASK-ADS-001, TASK-AUTH-003, TASK-LEGAL-001]
blocks: []
effort_hours: 8
new_files:
...
disallowed_tools:
  - "LevelPlay / AppLovin / Adjust / AppsFlyer / Adjust on kids SKU (TASK-LEGAL-001 §1.5(a))"
...
risk_if_skipped: "TASK-LEGAL-001 §1.5(d) — kids SKU MUST use only kWS contextual ads. Without this task, kids SKU has no compliant monetization path."
...
§1.2  **Lower cap than 13+.** Kids SKU max 3 rewarded views per day (vs 8 for 13+ per TASK-ADS-001). Plan §PART 6 + kid-app guidelines.
...
§1.7  **Parental override.** Per TASK-SUB-002 §1.8, parent can fully disable ads via Family tier dashboard. When disabled → 403 `ads.parental_disabled`.

§1.8  **Endpoint — kid rewarded request.** `POST /v1/ads/kids/rewarded/request` — separate from TASK-ADS-001 endpoint. Validates kids-SKU + parental override.

§1.9  **kWS SSV.** Similar to TASK-ADS-001 §1.11 — kWS server-side validation callback.
...
§1.12  **No ad before onboarding** (TASK-PET-004) + no ads in first 10 minutes of any session (kids-app industry pattern).
...
§1.14  **Reward delivery.** Per TASK-ECON-001 — 30 Coins granted via ledger after kWS SSV.

§1.15  **No social-comparison surfaces.** Per TASK-LEGAL-003 + kid-app guideline. No "X friends watched ads today" UX.

§1.16  **Replay protection.** Same as TASK-ADS-001 §1.12 + extended view_id uniqueness.
...
§1.18  **Tenant scoping.** Per TASK-INFRA-003 tenant model — B2B tenant kids ads use tenant's kWS sub-workspace if configured.

§1.19  **Sentry monitoring.** SDK errors + SSV mismatches alert immediately.

§1.20  **Analytics.** `ads.kids.rewarded.requested`, `ads.kids.rewarded.completed`, `ads.kids.cap_hit`, `ads.kids.parental_disabled` per TASK-OBS-001.

---

## §2 — Why this design

**Why kWS.** Plan §PART 6 + plan §PART 8 — only ad SDK certified contextual-only for under-13. Required by TASK-LEGAL-001 §1.5(d).
...
**Why parental override.** Plan §PART 8 TASK-SUB-002 — Family tier offers full ad disable.
...
```typescript
describe('TASK-ADS-002 — kids contextual', () => {
...
| 3 | LevelPlay SDK on kids binary | TASK-LEGAL-003 §1.10 inspection | Build blocked | Verified |
...
**Cross-reference:** TASK-ADS-001 13+ rewarded; TASK-SUB-002 parental override.
