...
created: 2026-05-17
shipped: 2026-05-17
related_frs: [TASK-INFRA-001, TASK-ART-001, TASK-PET-003, TASK-CARE-001, TASK-CARE-002, TASK-OBS-001, TASK-I18N-001, TASK-LEGAL-001]
depends_on: [TASK-INFRA-001, TASK-ART-001]
blocks: []
effort_hours: 12
new_files:
...
§1.3  **Tap targets.** Minimum 44×44 pt (Apple) / 48 dp (Android) for all interactive elements. Kids SKU 88×88 pt minimum per TASK-LEGAL-003 §1.11.

§1.4  **Reduce-motion mode.** App MUST detect `prefers-reduced-motion` (Cocos `sys.os` + native bridge) and:
- Cap idle animation frequency (per TASK-ART-001 §1.18).
- Replace evolution morph cuts with fade.
- Skip particle bursts > 6 particles (TASK-CARE-002 §1.12).
- Disable parallax / screen-shake.
- Disable AR camera-shake (TASK-AR-001 §1.14).

§1.5  **Colour-blind palette swap.** Settings → Accessibility → Colour-blind preset selector with options: protanopia / deuteranopia / tritanopia / default. When set, all colour-coded UI (stat bars per TASK-PET-003 §1.11, tier badges per TASK-PET-006) uses adjusted palette.
...
§1.10  **Skip-link for repetitive content.** "Skip animation" appears on the hatch ceremony (after midpoint per TASK-PET-004 §1.12) + any long sequence.

§1.11  **Audio cues alongside visual.** Every primary feedback also plays audio (mute-respect per TASK-ART-001 §1.14). E.g. feeding plays "om-nom" SFX in addition to animation.
...
§1.13  **Closed captions on AI dialogue.** Per TASK-AI-001, dialogue text is always visible (no audio-only delivery). For TTS-read mode (opt-in), captions remain.
...
§1.16  **Language-aware text expansion.** UI layouts MUST tolerate 30% text expansion (e.g. German ≈ 1.3× English). Per TASK-I18N-001 9-language wave.
...
§1.18  **Kids SKU specific.** Per TASK-LEGAL-003 — kids gets 1.3× base font + 88×88 pt taps. Apple Kids Category baseline.

§1.19  **Tenant-aware a11y override.** B2B tenants (TASK-B2B-001) inherit baseline; may not REMOVE accessibility features, only add.

§1.20  **Analytics.** `a11y.reduce_motion.enabled`, `a11y.colour_blind.preset_changed`, `a11y.dyslexia_font.enabled` per TASK-OBS-001 — informs prioritisation.
...
```typescript
describe('TASK-A11Y-001 — WCAG-AA', () => {
...
- **OQ-4 (resolved):** Kids 1.3× font? → §1.18 — per TASK-LEGAL-003.
...
**Cross-reference:** TASK-ART-001 reduce-motion paths; TASK-PET-003 colour-blind palette; TASK-LEGAL-003 kids tap-target sizing; TASK-I18N-001 text expansion.
