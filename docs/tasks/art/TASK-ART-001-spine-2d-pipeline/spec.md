...
created: 2026-05-17
shipped: 2026-05-17
related_frs: [TASK-INFRA-001, TASK-PET-001, TASK-PET-002, TASK-PET-006, TASK-CARE-001, TASK-CARE-002, TASK-CARE-003, TASK-AR-001, TASK-VIRAL-001, TASK-VIRAL-003, TASK-ECON-003, TASK-A11Y-001, TASK-B2B-001]
depends_on: [TASK-INFRA-001]
blocks: [TASK-PET-001, TASK-PET-002, TASK-PET-006, TASK-CARE-001, TASK-CARE-002, TASK-CARE-003, TASK-AR-001, TASK-VIRAL-001, TASK-ECON-003]
effort_hours: 14
new_files:
...
disallowed_tools:
  ...
  - "Live2D for the launch hero pet (heavier license; permitted for premium-species pets in a future task)"
...
risk_if_skipped: "Without a locked Spine authoring contract, every pet species takes 3× the artist budget (plan §PART 5 risk); the visual ceiling that differentiates from Pou-clones collapses; AR/TikTok-export hooks (TASK-AR-001, TASK-VIRAL-001) cannot ship because they depend on the contract's pose set."
...
ar_greet
```
20 animations exactly. Adding a 21st requires an task amendment (the count is the contract). Missing one breaks the linter.
...
```
On iOS, MUST use CoreHaptics `CHHapticEngine`; on Android API 26+, MUST use `VibrationEffect`; on web + older Android, MUST be a no-op (graceful degrade). Haptics MUST respect the user's accessibility "reduce motion" preference (per TASK-A11Y-001) and the OS-level haptics-off setting.
...
§1.17  **Per-pet tenant override.** A per-tenant theme bundle (TASK-B2B-001) MAY override colour palette / accessory presets via Spine's skin mechanism, but MUST NOT override the contract animation set. Tenant overrides live under `apps/cocos/assets/tenants/<slug>/pets/<species>/skin.json`. The tenant skin path is resolved by `SpineLoader.loadPetSkeleton` via the `tenantSlug` option (TASK-INFRA-001 §1.6 loader API).

§1.18  **A11Y reduce-motion compatibility.** When `prefers-reduced-motion` is set (per TASK-A11Y-001), `playContractAnimation` MUST: (a) replace evolutionary morph cuts with a fade; (b) cap idle loop frequency to 1× per 5 s; (c) skip particle layers attached to the Spine skeleton. The reduce-motion path MUST NOT break the contract — every animation still runs, just gentler.
...
**Why a 20-animation contract.** Without a contract, each new pet species drifts in animation names + counts, which means every UI consumer (Spine player invocation in `TASK-PET-002` evolution flow, `TASK-CARE-001` feed action, `TASK-AR-001` AR-greet, etc.) needs per-species switch cases. With a contract, the consumer code is generic and a new species only requires the Spine artist to fill the contract. The 20 names cover every gameplay-driven animation in P1-P3 — anything new requires an task amendment so the change is intentional.
...
**Why slot taxonomy excludes `body` and `face`.** Cosmetic swapping on `body` would let a player buy "make this pet's species look like a different species" — the moment that ships, the pet identity collapses and TASK-PET-006 rarity tiers become meaningless. `accessory_head` + `accessory_body` give cosmetic surface without breaking identity.
...
**Why deterministic texture compression.** `oxipng -o4` is the standard reproducible PNG optimiser. Non-deterministic compressors (some node-canvas builds) break TASK-INFRA-001 §1.13 reproducible builds.
...
**Why a tenant-skin override exists but cannot override contract animations.** Per TASK-B2B-001, tenants want a Pet-of-Techcombank look. Letting them swap the palette is a config; letting them remove animations would break consumer features (every consumer feature relies on the 20 animations existing).
  ...
  private assertContract(data: sp.SkeletonData): void {
    ...
    for (const required of ANIMATION_CONTRACT) {
      if (!present.has(required)) {
        throw new Error(`TASK-ART-001 §1.2 — animation '${required}' missing from skeleton`);
      }
    }
  }
...
if (errors.length) { console.error('TASK-ART-001 contract failures:\n  ' + errors.join('\n  ')); process.exit(1); }
console.log(`OK: ${path}`);
...
**AC13.** Reduce-motion mode is respected — when `prefers-reduced-motion: reduce` is set (per TASK-A11Y-001), `playContractAnimation` skips particle layers + falls back to fade for evolution morphs. Verified by `__tests__/AnimationContract.spec.ts`.
...
describe('TASK-ART-001 — animation contract', () => {
...
describe('TASK-ART-001 §1.10/§1.14 — AudioMixer defaults', () => {
...
**Internal:** TASK-INFRA-001 (Cocos scaffold + asset-bundle loader + bundle-budget script).

**Blocks:** TASK-PET-001 (pet entity needs the skeleton runtime), TASK-PET-002 (evolution stages use the skin presets), TASK-PET-006 (5 launch species all conform to contract), TASK-CARE-001/002/003 (each care action triggers a contract animation), TASK-AR-001 (AR-greet animation), TASK-VIRAL-001 (TikTok export captures contract animations), TASK-ECON-003 (UGC Pet Couture uses accessory slots).
...
```text
$ pnpm art:lint
TASK-ART-001 contract failures:
...
- **OQ-1 (resolved):** Spine Essential vs Pro? → §`allowed_tools` — Essential ($69) sufficient for launch; Pro ($299) only if mesh skinning becomes needed for premium pets (deferred task).
- **OQ-2 (resolved):** Live2D for the hero pet? → §`disallowed_tools` + §2 — Live2D permitted only for premium-species pets in a future task; launch hero uses Spine.
- **OQ-3 (resolved):** Contract size — 20 vs 25 animations? → §1.2 + §2 — 20 covers every P1-P3 gameplay-driven animation; new ones require task amendment.
...
| 4 | Spine 4.2 → 4.3 major upgrade breaks runtime | Cocos plugin regression | Pet rendering breaks | Pin Spine + runtime; upgrade task with regression pass |
...
| 10 | Performance test fails on 1 GB Android emulator | `__tests__/perf-spine.spec.ts` fails | PR blocked | Optimise skeleton bone count; reduce mesh deformation; or relax target with explicit task amendment |
...
**Cross-reference:** This task is the substrate for every visual gameplay task in P1. Without it, TASK-PET-001 cannot load a renderable pet; TASK-CARE-001 has nothing to animate; TASK-VIRAL-001 has no export-able motion. TASK-B2B-001 (tenant themes) extends this without breaking the contract.
