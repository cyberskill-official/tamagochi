...
created: 2026-05-17
shipped: 2026-05-17
related_frs: [TASK-ECON-001, TASK-ECON-003, TASK-LEGAL-002, TASK-LEGAL-003, TASK-PET-006, TASK-SUB-001, TASK-AUTH-001, TASK-AUTH-003, TASK-INFRA-003, TASK-I18N-002, TASK-OBS-001]
depends_on: [TASK-ECON-001, TASK-LEGAL-002]
blocks: [TASK-ECON-003, TASK-SUB-001, TASK-VIRAL-004]
effort_hours: 14
new_files:
...
allowed_tools:
  ...
  - "Stripe + Paddle for web B2B (TASK-B2B-003 path)"
disallowed_tools:
  - "Random-outcome IAP (no loot boxes per TASK-LEGAL-002 §1.1)"
  - "Bundle IAP with randomised pet pulls"
  - "IAP gating during onboarding (Apple Guideline 3.1.3(b) per TASK-PET-004 §1.7)"
  - "Surprise eggs sold for real money"
  - "Client-validated receipts (server-side validation only)"
risk_if_skipped: "Plan §PART 6 + plan §PART 7 VN soft launch — without IAP catalogue with VN-payment rails, the consumer game cannot monetize. TASK-SUB-001 + TASK-VIRAL-004 + TASK-ECON-003 all depend on this."
audience_age_gate: "13+"
---

## §1 — Description (BCP-14 normative)

§1.1  **IAP catalogue — direct purchase only.** Every IAP product MUST have a deterministic outcome. NO randomised inventory pulls. Per TASK-LEGAL-002 §1.1.
...
- On valid: grants entitlement via TASK-ECON-001 ledger (`iap_source` account type).
...
§1.11  **No IAP during onboarding.** Per TASK-PET-004 §1.7 + Apple Guideline 3.1.3(b), IAP store UI MUST NOT surface until 5+ minutes after onboarding tutorial dismiss.
...
- Server processes refund: revokes entitlement (per TASK-ECON-001 ledger `iap_source` reversal).
- If the player has already used the Hearts → flag for DPO review (no clawback if used).

§1.14  **Family Sharing approval (Apple) + Family Library (Google).** Per TASK-LEGAL-003 §1.6 — IAP for under-13 needs parental approval. Kids SKU + TASK-SUB-002 Family tier wires this.
...
§1.18  **Anti-fraud — receipt replay.** Apple/Google have known receipt-replay vulnerabilities. Antom has VN-specific patterns. Per TASK-OBS-002 anti-cheat — flag receipts older than 7 days as suspicious + DPO review.

§1.19  **BE/NL safe mode.** Per TASK-LEGAL-002 §1.12 — even though no real-money randomisation exists, IAP marketing UI hidden if "surprise" language present in copy. Surprise-egg UI (Coins-only) already hidden per TASK-LEGAL-002.

§1.20  **Analytics.** `iap.purchase.initiated`, `iap.receipt.validated { vendor }`, `iap.purchase.completed { product_id, price_usd, vendor }`, `iap.refund.processed`, `iap.validation.failed { reason }` per TASK-OBS-001.

---

## §2 — Why this design

**Why direct-purchase only.** Plan §PART 8 + TASK-LEGAL-002 — Belgium/NL/EU loot-box compliance + HoYoverse precedent.
...
**Why Family Sharing for kids.** TASK-LEGAL-003 §1.6 — parental approval required.
...
- **OQ-4 (resolved):** Family Sharing? → §1.14 — wired via TASK-LEGAL-003.
...
**Cross-reference:** TASK-ECON-001 ledger, TASK-SUB-001 subscription, TASK-VIRAL-004 battle pass, TASK-PET-006 premium species.
