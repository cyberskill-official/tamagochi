...
created: 2026-05-17
shipped: 2026-05-17
related_frs: [TASK-LEGAL-001, TASK-LEGAL-003, TASK-PET-006, TASK-PET-007, TASK-PET-008, TASK-ECON-001, TASK-ECON-002, TASK-SUB-001, TASK-VIRAL-004]
depends_on: [TASK-LEGAL-001]
blocks: [TASK-PET-006, TASK-PET-008, TASK-ECON-002, TASK-VIRAL-004]
effort_hours: 6
new_files:
...
§1.5  **Pet rescue MUST NOT cost real money.** Per TASK-PET-008 (Permadeath-Lite), a pet that has "moved to grandma's house" MUST be rescuable via a free daily-ritual path only. No real-money revival, no real-money cooldown skip, no real-money rescue accelerator. (HoYoverse $20M FTC settlement is the cited precedent.)

§1.6  **Breeding MUST be deterministic given inputs.** Per TASK-PET-007 (Breeding), the offspring trait inheritance MUST be a pure function of the two parent pets' palette + stat bias + a server-side seed deterministically derived from the parent ids. The function MUST be auditable post-hoc by replaying the inputs. Breeding MUST NOT consume real-money currency.

§1.7  **Battle pass MUST be linear / disclosed.** Per TASK-VIRAL-004, every tier reward MUST be deterministically known at purchase time. A "mystery tier" reward shape is forbidden.
...
§1.14  **Annual review trigger.** This task's policy MUST be re-evaluated whenever any of: (a) a new EU member-state issues guidance on loot boxes; (b) EU Digital Fairness Act passes parliament; (c) a new app store category emerges requiring deeper disclosure; (d) a competitor receives a regulator action — using the same trigger mechanism as TASK-LEGAL-001 §1.14.
...
export class RandomisationDisclosureGuard implements CanActivate {
  ...
  canActivate(ctx: ExecutionContext): boolean {
    ...
    if (!(req as any).disclosureShownEventId) {
      throw new Error('disclosure not shown before randomisation invocation (TASK-LEGAL-002 §1.3)');
    }
    return true;
  }
}
...
module.exports = {
  meta: {
    ...
    messages: {
      forbidden: 'TASK-LEGAL-002 §1.10 — randomisation function called in a path that includes an IAP receipt; loot-box-adjacent code is not allowed.',
    },
  },
  create(context) {
    return {
...
describe('TASK-LEGAL-002 §1.9 — drop-rate deviation', () => {
  beforeAll(async () => {
...
**Internal:**
- TASK-LEGAL-001 (compliance baseline must be in place — DPO and Safe Harbor are foundational).

**Blocks:** TASK-PET-006 (rarity tiers must obey this policy), TASK-PET-008 (Permadeath-Lite revival must be free), TASK-ECON-002 (IAP catalogue must lint-pass), TASK-VIRAL-004 (battle pass must be linear).
...
apps/api/src/iap/pet-purchase.service.ts
  72:12  error  TASK-LEGAL-002 §1.10 — randomisation function called in a path that includes an IAP receipt; loot-box-adjacent code is not allowed.  no-real-money-random-pull
```
...
FAIL: product "premium-egg-pack" has outcome_type "random"; TASK-LEGAL-002 §1.11 requires outcome_type === "deterministic".
exit 1
```
...
| 9 | EU DFA passes and changes the goalposts | Annual review trigger + news watch | Existing earned-currency mechanics may need refresh | Re-author this task; ship policy patch within the regulator's transition window |
...
- The set of items behind each rarity weight will be refreshed via TASK-PET-006; this task locks the framework, not the inventory.
...
**Cross-reference:** This task is the substrate for TASK-PET-006 (rarity tiers), TASK-PET-007 (deterministic breeding), TASK-PET-008 (free rescue), TASK-ECON-002 (no random IAP), TASK-VIRAL-004 (linear battle pass). Without this in place, those tasks cannot be authored coherently.
