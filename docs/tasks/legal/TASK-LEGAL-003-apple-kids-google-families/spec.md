...
created: 2026-05-17
shipped: 2026-05-17
related_frs: [TASK-LEGAL-001, TASK-LEGAL-002, TASK-AUTH-003, TASK-ADS-002, TASK-AI-002, TASK-OBS-001, TASK-SUB-002]
depends_on: [TASK-LEGAL-001, TASK-LEGAL-002]
blocks: [TASK-ADS-002, TASK-SUB-002, TASK-VIRAL-005]
effort_hours: 6
new_files:
...
allowed_tools:
  ...
  - "SuperAwesome kWS contextual-ad gate (depends on TASK-ADS-002)"
disallowed_tools:
...
§1.3  **Two bundle IDs / two packages.** Per TASK-LEGAL-001 §1.5, the kids SKU and 13+ SKU MUST ship as separate App Store records and separate Play packages. Fastlane / Gradle MUST build two binaries from one Cocos codebase using a `build_target=kids` or `build_target=standard` flag. The flag MUST drive: bundle ID, app name (`Tamagochi Kids` vs `Tamagochi`), icon, splash screen, in-app feature flags, and SDK include list.
...
if (hits.length > 0) {
  console.error(`TASK-LEGAL-003 §1.5 fail — forbidden SDK strings in kids binary: ${hits.join(', ')}`);
  process.exit(1);
}
...
describe('TASK-LEGAL-003 §1.4 — external-link gate', () => {
...
**Internal:**
- TASK-LEGAL-001 (compliance baseline; DPO + Safe Harbor must already be in place).
- TASK-LEGAL-002 (loot-box-free posture must already be in place to satisfy Apple/Google kids-policy IAP requirements).

**Blocks:** TASK-ADS-002 (kWS contextual-only gate implementation), TASK-SUB-002 (Family tier — Family Sharing wiring), TASK-VIRAL-005 (push under-13 quiet-hours expansion).
...
TASK-LEGAL-003 §1.5 fail — forbidden SDK strings in kids binary: AppLovin, AppsFlyer
exit 1
```
...
$ pnpm legal:check
docs/legal/store-listing-kids-sku-en.md
  line 14: "Tamagotchi-style pet care" — TASK-LEGAL-003 §1.8 forbids the word "Tamagotchi" (Bandai trademark).
exit 1
```
...
| 11 | Parental gate locale missing for new launch language | Locale QA gap | Gate falls through | Block release of the new locale until gate translated; add to the I18N task's pre-launch checklist |
...
**Cross-reference:** This task is the gating substrate for submission of the kids SKU. Without all checklists complete + binary inspection green + parental gate working + store listing Bandai-clean, the kids SKU cannot ship and TASK-AUTH-003 / TASK-AI-002 / TASK-ADS-002 / TASK-SUB-002 / TASK-VIRAL-005 cannot land in production.
