...
created: 2026-05-17
shipped: 2026-05-17
related_frs: [TASK-LEGAL-002, TASK-LEGAL-003, TASK-AUTH-003, TASK-AI-002, TASK-ADS-002, TASK-OBS-001, TASK-VIRAL-005, TASK-SUB-002, TASK-B2B-002]
depends_on: []
blocks: [TASK-LEGAL-002, TASK-LEGAL-003, TASK-AUTH-003, TASK-AI-002, TASK-ADS-002, TASK-OBS-001]
effort_hours: 14
new_files:
...
disallowed_tools:
  ...
  - "Generative LLM chat surfaces in under-13 SKU — must use scripted dialogue trees (see TASK-AI-002)"
...
This task establishes the compliance baseline that **all kid-touching tasks depend on**. It MUST be in place before any task that processes user-personal data ships, and it gates every later phase.
...
§1.5  **COPPA-2025 separate-SKU policy.** Under-13 users MUST be served via a **separate Apple App Store SKU and Google Play package** (`world.cyberskill.tamagochi.kids`) distinct from the 13+ SKU (`world.cyberskill.tamagochi`). The kids SKU MUST: (a) disable all behavioural analytics SDKs (only GameAnalytics + Sentry permitted per TASK-OBS-001); (b) disable generative LLM dialogue (scripted trees only per TASK-AI-002); (c) restrict friend-graph features to invite-code-only — no name search (per TASK-SOCIAL-001); (d) restrict ads to SuperAwesome kWS contextual-only (per TASK-ADS-002); (e) restrict push notifications to a curated engagement-safe template set with the under-13 quiet-hours window expanded to local 20:00-08:00 (per TASK-VIRAL-005); (f) require parental consent before any data persistence beyond a session-scoped device ID.
...
§1.14  **Compliance program lifecycle.** The Founder MUST review this task's artefacts annually and re-author the DPIA + TIA when any of: (a) a new cross-border vendor is added; (b) a new data category is collected; (c) a regulator updates its guidance materially (e.g. EU Digital Fairness Act publication, COPPA-2025 sub-regulation, PDPL Decree update); (d) the user-base crosses 100K users (PDPL small-business grace-period exit threshold per `docs/legal/coppa-2025-readiness-checklist.md` §3).
...
```sql
-- Per TASK-LEGAL-001 §1.6 and §1.12 — kids SKU lives in its own Supabase project.
...
**AC12.** Annual review reminder: a scheduled task (`mcp__scheduled-tasks__create_scheduled_task`) fires on 2027-05-17 with the prompt "Annual review of TASK-LEGAL-001 artefacts due — re-author DPIA + TIA if any trigger condition (a-d) has occurred." Verified by inspecting the scheduled-task list.
...
describe('TASK-LEGAL-001 — parental consent flow', () => {
...
describe('TASK-LEGAL-001 — CA05 form generator', () => {
  const payload = {
...
- Anthropic Claude US for LLM (cross-border declared in TIA; under-13 disabled per TASK-AI-002).
- Sentry Germany region for errors (chosen specifically for EU customer assurance).

**Internal**
- None — this is the root task with `depends_on: []`.

**Blocks** (the tasks that cannot start until this lands)
- TASK-LEGAL-002 (loot-box policy depends on this baseline)
- TASK-LEGAL-003 (store-category declarations depend on the SKU split here)
- TASK-AUTH-003 (invite-code flow depends on parental consent service)
- TASK-AI-002 (kids LLM gate)
- TASK-ADS-002 (kWS contextual gate)
- TASK-OBS-001 (analytics SDK allow/deny list)
- TASK-VIRAL-005 (push notification kid quiet-hours)
...
- **OQ-1 (resolved):** PRIVO vs SuperAwesome kWS? → Decision deferred to `safe-harbor-vendor-decision-memo.md` ANNEX A (mandatory pre-launch decision). Both options spec'd in §1.6 so neither blocks downstream tasks.
...
- **OQ-5 (resolved):** Kids SKU LLM allowed? → §1.5(b) forbids; scripted trees in TASK-AI-002.
...
**Risk register cross-reference:** This task is the mitigation for plan §PART 10 risks #1 (regulator action under COPPA / PDPL) and the foundation for #4 (Apple/Google store policy changes for kids category).
