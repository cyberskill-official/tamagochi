...
created: 2026-05-17
shipped: 2026-05-17
related_frs: [TASK-PET-001, TASK-PET-005, TASK-PET-007, TASK-PET-008, TASK-LEGAL-002, TASK-ART-001, TASK-CARE-005, TASK-ECON-001, TASK-ECON-002, TASK-OBS-001, TASK-SOCIAL-002]
depends_on: [TASK-PET-005, TASK-LEGAL-002, TASK-ART-001]
blocks: [TASK-PET-007, TASK-SOCIAL-003]
effort_hours: 12
new_files:
...
allowed_tools:
  - "TASK-LEGAL-002 deterministic-only randomisation"
  - "TASK-CARE-005 streak rewards"
...
risk_if_skipped: "Plan §PART 3 — multi-species + rarity tiers are the canonical f2p meta. Tamagotchi Paradise (2025) shows breeding+variations is the proven hook. Without 5 species × rarity, breeding (TASK-PET-007) has no genetic surface."
audience_age_gate: "any"
---

## §1 — Description (BCP-14 normative)

§1.1  **5 launch species** (per plan §PART 3): **Mochi** (cat-like, brown/cream), **Pengu** (penguin), **Bao** (dumpling-shaped), **Fluffit** (rabbit-like), **Tako** (octopus). Each species MUST have a Spine skeleton conforming to TASK-ART-001 §1.2 20-animation contract.
...
§1.3  **Acquisition paths — earnable only**, per TASK-LEGAL-002:
- **Common** (Mochi tier 1): default hatch outcome — every new player's first pet.
- **Rare**: streak rewards at 7-day milestone (TASK-CARE-005 §1.16), surprise eggs (earned Coins, drop rates disclosed).
- **Epic**: event quests (P3+ live-ops), 30-day streak.
- **Mythic**: co-parent milestones (TASK-SOCIAL-002 P2), 100-day streak.
- **Legendary**: breeding (TASK-PET-007), 365-day streak.

**No real-money path to any rarity**. Premium species (P3 IAP via TASK-ECON-002) are **direct purchase only** — not randomised.
...
§1.5  **Drop rates (disclosed).** Surprise-egg mechanic (TASK-LEGAL-002 §1.2 earned-currency randomisation):
...
Published at `docs/legal/rarity-drop-rates-public.json` per TASK-LEGAL-002 §1.2.
...
§1.7  **Tier display in UI.** Pet roster MUST display tier badge (colour-coded: gray/blue/purple/gold/rainbow). Trade UX (TASK-SOCIAL-003) MUST show tier prominently.

§1.8  **Tier-based persona modulation.** TASK-AI-001 persona prompts MUST include tier as context. Legendary pets MAY have slightly more confident dialogue patterns (designer-locked).

§1.9  **Tier-based stat modulation.** Higher-tier pets MAY have slightly slower stat decay (-10% per tier above common, capped at -30% for legendary). TASK-PET-003 `STAT_CONFIG` extended to read `pet.tier`.

§1.10  **Reset to Common on grandma rescue.** A pet that goes to `grandma_house` (TASK-PET-008) and is rescued retains its species + tier. No tier downgrade on rescue.
...
§1.13  **Surprise egg mechanic.** `POST /v1/eggs/surprise-egg/open` consumes 500 Coins (TASK-ECON-001 stubbed), rolls per §1.5 drop rates, returns the resulting species+tier. Per TASK-LEGAL-002 §1.4, outcome receipt with server_seed_hash recorded.

§1.14  **Drop-rate disclosure UI.** Before opening a surprise egg, UX shows tier-percentage breakdown + link to full rates page. Per TASK-LEGAL-002 §1.3.

§1.15  **BE/NL marketing-hide.** Per TASK-LEGAL-002 §1.12, surprise-egg marketing UI hidden in BE/NL until counsel confirms.
...
§1.18  **Per-species asset budget.** Per TASK-INFRA-001 §1.5: ≤ 1.5 MB per species. The 5-palette-per-species shipping requires careful texture compression.

§1.19  **Cocos `SpeciesUnlockUi.ts`.** When a player acquires their first-ever pet of a new species, a one-time "Species unlocked!" celebration UX plays (TASK-ART-001 `wave` + Lottie sparkles).

§1.20  **B2B tenant species overrides.** Per TASK-B2B-001 (P4), a tenant MAY override palette + accessory presets via Spine skin but MUST NOT add new species or new tiers. Tenant overrides ride atop this task's species + tier model.
...
**Why all earnable (no real-money pulls).** Plan §PART 8 + TASK-LEGAL-002. Real-money randomised rarity is the canonical loot-box pattern that Belgium / NL / EU + HoYoverse precedent forbid for kid-skewing apps.

**Why drop rates disclosed.** Apple Guideline 3.1.1 + Google Play policy + TASK-LEGAL-002 §1.2 + EU DFA preparedness.
...
export class RarityAcquisitionService {
  async openSurpriseEgg(userId: string): Promise<{ species: Species; tier: Tier; outcome_receipt_id: string }> {
    await this.disclosure.assertShown(userId, 'surprise-egg');  // TASK-LEGAL-002 §1.3
    await this.region.assertNotBeNl(userId);                     // TASK-LEGAL-002 §1.12
    const cost = 500;
...
```typescript
// apps/api/src/pets/species/__tests__/rarity-acquisition.spec.ts
describe('TASK-PET-006 — rarity acquisition', () => {
...
| 2 | Surprise-egg accidentally allows real-money | TASK-LEGAL-002 lint | Build blocked | Remove IAP gate; verify |
...
| 6 | Tier-decay modifier breaks stat reconciliation | TASK-PET-003 test | Stat math wrong | Modifier applied consistently in both decay + reconciliation |
...
| 12 | Tenant override breaks rarity tiers | TASK-B2B-001 validation | Tenant theme rejected | Schema lint |
...
**Cross-reference:** TASK-PET-007 breeding output picks species+tier; TASK-SOCIAL-003 displays tier; TASK-ECON-002 direct premium-species purchase (not random).
