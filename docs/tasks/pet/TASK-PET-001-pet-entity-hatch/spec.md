...
created: 2026-05-17
shipped: 2026-05-17
related_frs: [TASK-INFRA-002, TASK-INFRA-003, TASK-ART-001, TASK-AUTH-001, TASK-AUTH-003, TASK-PET-002, TASK-PET-003, TASK-PET-004, TASK-PET-005, TASK-PET-006, TASK-CARE-001, TASK-AI-002, TASK-OBS-001, TASK-B2B-002]
depends_on: [TASK-INFRA-003, TASK-INFRA-002, TASK-ART-001]
blocks: [TASK-PET-002, TASK-PET-003, TASK-PET-004, TASK-CARE-001, TASK-CARE-002, TASK-CARE-003, TASK-CARE-004, TASK-AR-001]
effort_hours: 10
new_files:
...
allowed_tools:
  - "Supabase Postgres + RLS (TASK-INFRA-003)"
  - "Colyseus 0.16 + @colyseus/schema"
  - "OpenAI Moderation API (content-safety for names — TASK-AI-002 wires the under-13 stricter mode)"
...
disallowed_tools:
  ...
  - "Hatching a pet on the kids SKU without verified parental consent (TASK-AUTH-003 gate)"
...
§1.1  **Pet entity schema (Postgres).** Pets MUST persist in a `public.pets` table on the appropriate Supabase project (per TASK-INFRA-003 §1.1, kids pets live in the kids project, standard pets in the standard project). Columns:
```
...
```
Composite indexes on `(tenant_id, owner_id)` and `(tenant_id, status)`. RLS policy `pets self+co-parent read` allows `owner_id = auth.uid()` OR co-parent join (TASK-SOCIAL-002 will extend).
...
§1.3  **Colyseus PetState schema.** The Colyseus room state shape (per TASK-INFRA-002 §1.11) MUST include a `PetState` decorated with `@type` schema fields matching the Postgres columns the client needs to render: `id, ownerId, species, stage, displayName, palette_seed, hunger, cleanliness, happiness, energy, lastActionId`. Stats (hunger / cleanliness / happiness / energy) live in `PetState` but their persistence to Postgres is TASK-PET-003.
...
- Server validates: player has < 3 pets (free-tier cap from TASK-PET-005), audience age gate matches, no recent hatch within 1 hour (debounce);
...
§1.6  **Palette seed.** `palette_seed` MUST be a deterministic random 16-char string generated server-side at hatch. The Spine palette (per TASK-ART-001 §1.4 accessory slots) is derived from the seed via a published palette-generator function `derivePalette(seed)`. This makes every pet visually unique-ish at hatch, and reproducible (a support ticket can recover what the player saw). For TASK-VIRAL-003 (generative pet from prompt/selfie), the palette_seed is overridden by the generator output.

§1.7  **Co-parent prep.** The schema MUST support a future co-parent relationship (TASK-SOCIAL-002). A `pet_co_parents` join table is provisioned at this task with columns `(pet_id, co_parent_user_id, added_at, status check in ('pending','active','removed'))`. The table is empty at P1 but its schema + RLS policy MUST be authored now to avoid migration thrash in P2.

§1.8  **Audience gate inheritance.** A pet's `audience_age_gate` MUST inherit from its owner's `app_users.audience_age_gate` at creation time. The COPPA age-gate middleware (TASK-LEGAL-001 §1.10) MUST allow a pet's owner to access their own pet regardless of state (so a child whose parent revoked consent can still see "your pet has gone to grandma's" UX during the 30-day deletion grace).

§1.9  **`PlayerPetsRoster` state.** The Colyseus `PlayerPetsRoster` schema MUST hold a `MapSchema<string, PetState>` keyed by pet id, with operations: `addPet(pet)`, `removePet(petId)`, `evolvePet(petId, newStage)`. The roster is loaded from Postgres on player join (TASK-INFRA-002 `onJoin`) and persisted back on idle disposal (TASK-INFRA-002 §1.12).
...
§1.11  **Last-seen tracking.** `last_seen_at` MUST be updated on every meaningful interaction (feed, clean, hug, mini-game, AR, name change). Decay timers (TASK-PET-003) read from this column; TASK-PET-008 (Permadeath-Lite) uses it as the 7-day-neglect signal.
...
§1.14  **Pet entity audit.** Every pet state transition (`hatch`, `name_changed`, `stage_changed`, `grandma`, `tombstoned`) MUST emit a row in `pet_audit_log` table (TASK-INFRA-003 §1.12). Retention 7 years for kids project (COPPA), 2 years for standard.

§1.15  **Postgres → Colyseus reconciliation.** On player rejoin of a PetRoom, the server MUST: (a) fetch all `active` pets for the player from Postgres; (b) populate the Colyseus `PlayerPetsRoster`; (c) reconcile stat-bar decay (TASK-PET-003) by computing time since `last_seen_at` and applying decay; (d) broadcast updated state to the client.

§1.16  **Pet deletion on owner deletion.** `pets.owner_id` has `on delete cascade`. When a player's account is deleted (TASK-AUTH-001 §1.19 or TASK-LEGAL-001 §1.9), all their pets are cascade-deleted, AND a tombstone row is added to `pet_audit_log` recording the deletion reason.

§1.17  **No public pet listings without privacy review.** A pet's `display_name` MUST NOT appear in any public/anonymous endpoint without explicit consent (a "make my pet public" toggle, deferred to a future task). Trade window (TASK-SOCIAL-003) is in-game and consent-gated.

§1.18  **Pet quota soft-cap.** Free tier allows 3 active pets per player; Pet+ subscription extends to 10 (TASK-PET-005 final). At this task's P1 scope, the cap is hard-coded at 3 with the entitlement check stubbed (returns "always free tier"). TASK-SUB-001 wires the real entitlement.

§1.19  **Tenant partition.** Per TASK-INFRA-003 §1.4, the `tenant_id` column MUST be populated at pet creation. Consumer pets get `tenant_id = 'mochi'`; PetOS B2B tenants (P4) get their own slug. Cross-tenant pet visibility is forbidden by RLS.

§1.20  **Cocos PetController component.** A Cocos component `PetController.ts` MUST attach to the rendered pet node and: (a) subscribe to the Colyseus `PetState` schema changes; (b) drive `SpineLoader.playContractAnimation` (TASK-ART-001) based on state transitions; (c) emit `pet.rendered.frame` analytics every 10 seconds for performance monitoring. The component MUST gracefully no-op when state is `egg` or `grandma_house`.
...
**Why `audience_age_gate` inherits from owner.** A kid who graduates to 13 (TASK-INFRA-003 §1.13 graduation flow) brings their pets forward; the audience_age_gate is re-derived at graduation time. Inheritance at hatch time keeps the model consistent.
...
**Why `last_seen_at` not `last_action_at`.** "Seen" includes passive viewing (open the pet screen, look at AR). TASK-PET-003 stat decay should restart on view, not require interaction — viewing is the "the player checked on the pet" signal.

**Why `on delete cascade`.** GDPR Article 17 + COPPA-2025 erasure require full removal. Cascade is the simplest correct behaviour; the audit row preserves the *fact* of deletion (TASK-LEGAL-001 §3.6).

**Why no public pet listings at P1.** Privacy-by-default. A public-pet toggle is a real privacy decision that needs its own task (probably P2 or later, gated by Trust & Safety review for the kids SKU).
...
### 3.2 — Colyseus `PetState` (extended from TASK-INFRA-002)
...
-- pet_co_parents prep for TASK-SOCIAL-002
...
describe('TASK-PET-001 — hatch flow', () => {
...
describe('TASK-PET-001 §1.5 — name content-safety', () => {
...
**Internal:** TASK-INFRA-003 (Supabase + RLS + tenant partition); TASK-INFRA-002 (Colyseus + JWT-verified onJoin); TASK-ART-001 (Spine pipeline + contract animations for the hatch ceremony).

**Blocks:** TASK-PET-002 (evolution stages — needs `stage` column + animation hooks); TASK-PET-003 (stat-bar decay reads `last_seen_at`); TASK-PET-004 (onboarding flow drives the hatch UX); TASK-CARE-001/002/003/004 (care actions mutate stat columns); TASK-AR-001 (AR placement renders the same skeleton).
...
- **OQ-3 (resolved):** Where lives stat-bar decay logic? → §1.11 + TASK-PET-003 — TASK-PET-003 owns decay rates; this task provides the `last_seen_at` substrate.
...
| 8 | Hatch quota soft-cap of 3 incorrect on Pet+ user | Entitlement service returns wrong tier | UX shows wrong message | TASK-PET-005 entitlement integration during P2 fixes; meanwhile manual cap override via DPO |
...
**Cross-reference:** This task is the substrate for the entire pet entity. TASK-PET-002/003/004 + every CARE task + every SOCIAL task + every ECON task reads/writes this schema.
