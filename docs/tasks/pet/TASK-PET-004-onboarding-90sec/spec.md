...
created: 2026-05-17
shipped: 2026-05-17
related_frs: [TASK-PET-001, TASK-PET-002, TASK-PET-003, TASK-ART-001, TASK-AUTH-001, TASK-AUTH-003, TASK-CARE-003, TASK-SOCIAL-002, TASK-OBS-001, TASK-I18N-001, TASK-LEGAL-001]
depends_on: [TASK-PET-001, TASK-PET-002, TASK-PET-003, TASK-ART-001]
blocks: [TASK-CARE-001, TASK-CARE-005]
effort_hours: 8
new_files:
...
allowed_tools:
  - "Cocos scene loader"
  - "Spine `hatch` + `idle_baby` + `care_pet` animations (TASK-ART-001 contract)"
  - "Lottie UI confetti / sparkles"
  - "Howler.js audio (hatch celebrate)"
  - "Haptics tap-light + success (TASK-ART-001 HapticsAdapter)"
disallowed_tools:
  ...
  - "Co-parent invite at P1 — stub only, fully enabled in P2 via TASK-SOCIAL-002"
...
1. **Hatch ceremony** — Egg animation (TASK-ART-001 `hatch`); ~20 seconds; auto-progresses;
2. **Name** — Text input + content-safety (TASK-PET-001 §1.5); player-paced;
3. **First pat** — Single big "tap your pet" cue, haptic + `care_pet` animation + +5 happiness (no stat displayed yet);
4. **Co-parent stub** — "Invite a grown-up / friend to share?" — at P1 this is a stub that records intent only and shows "Coming soon — we'll let you know!"; TASK-SOCIAL-002 in P2 wires the real action;
...
`duration_ms` is the time from `entered` to `completed` per step. Server-side aggregates per-cohort funnel via TASK-OBS-001 pipeline.
...
§1.10  **Localised copy.** Onboarding strings MUST live in `apps/cocos/assets/i18n/<locale>/onboarding.json` (per TASK-I18N-001). EN + VI MUST be authored before P1 release. Other locales follow in P4.

§1.11  **Under-13 SKU adaptations.** On the kids SKU, onboarding MUST:
- skip the co-parent step entirely (kids cannot independently invite anyone — parental dashboard controls friend additions in TASK-SUB-002);
...
- play `hatch` audio at 50% default volume (audio default is muted on kids SKU per TASK-ART-001 §1.14, so this only applies if the player manually enabled audio).
...
§1.13  **A11Y reduce-motion adaptation.** When `prefers-reduced-motion` is enabled (TASK-A11Y-001), the hatch ceremony plays the simplified path: fade-in instead of egg-shake; no confetti particles; static `idle_baby` skip directly to step 2.
...
§1.15  **AC analytics funnel.** A scheduled job MUST query the `onboarding_state` table daily to compute the P0-pipeline funnel: signups → step 1 entered → step 1 completed → step 2 ... → tutorial dismissed. Funnel rendered in the Grafana dashboard `pet-care-funnel.json` (TASK-OBS-001 §1.18).
...
§1.18  **Onboarding completion grants.** On tutorial dismiss, the player MUST receive: 100 soft Coins + the "First Pat" badge. Badges live in a future task (TASK-VIRAL-004 battle pass scaffold) but the badge slot is reserved here.
...
§1.20  **No third-party data collection during onboarding.** Per TASK-OBS-001 §1.17 (PII consent before identify), `Analytics.identify` is deferred to AFTER tutorial dismiss. During onboarding, all analytics are session-scoped (anonymous identifier) until the player has consented to the policy version.
...
**Why a stub for co-parent at P1.** TASK-SOCIAL-002 (PetPair co-parent) lands in P2. But the co-parent VIRAL hook is plan §PART 3 #1 — designing the placeholder at P1 ensures the slot exists, the storyboard locks in, AND the data on "what % of players said yes" tells us in advance whether to prioritise P2 PetPair work.
...
**Why server-side stat application during onboarding.** Per TASK-PET-003 §1.5 server-authoritative. The +25 happiness from first-pat is a real stat mutation, persisted from the start. No special onboarding-only state model — consistency with the production stat system.
...
**Why kids SKU skips co-parent step.** Kids cannot independently invite anyone (COPPA + TASK-SOCIAL-001 §1 + TASK-AUTH-003 design). Surfacing the co-parent invite on kids SKU would be confusing UX + a regulator question.
...
**Why 100 Coins + badge at completion.** Concrete reward = retention reinforcement. 100 Coins is enough to do 1-2 things in the first session but not enough to skip future engagement loops. Badge slot reserved for TASK-VIRAL-004 battle pass.

**Why analytics `identify` deferred until after tutorial dismiss.** TASK-OBS-001 §1.17 — policy-version stamping happens at sign-in (TASK-AUTH-001 §1.7), but `identify` to analytics vendors carries PII context. Deferring until after tutorial dismiss aligns with "first meaningful intent" — the player has demonstrably engaged with the product.
...
describe('TASK-PET-004 — onboarding state', () => {
...
**Internal:** TASK-PET-001 (pet created), TASK-PET-002 (stage transitions show in flow), TASK-PET-003 (first pat applies happiness), TASK-ART-001 (hatch + care_pet animations + haptics), TASK-AUTH-001/003 (signed-in player), TASK-OBS-001 (analytics + Sentry breadcrumbs), TASK-I18N-001 (locale bundles), TASK-LEGAL-001 (`identify` deferral).

**Blocks:** TASK-CARE-001 (first feed assumes pet is past onboarding), TASK-CARE-005 (streak system starts after tutorial dismiss).
...
**Cross-reference:** This task completes the PET slice (TASK-PET-001..004). With it, every P1 gameplay task can assume the player has a hatched + named + first-patted pet. TASK-CARE-001 picks up from "main game scene" after tutorial dismiss.
