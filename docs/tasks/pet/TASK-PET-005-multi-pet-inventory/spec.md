...
created: 2026-05-17
shipped: 2026-05-17
related_frs: [TASK-PET-001, TASK-PET-003, TASK-PET-006, TASK-PET-007, TASK-PET-008, TASK-SUB-001, TASK-SOCIAL-002, TASK-SOCIAL-003, TASK-ECON-001, TASK-OBS-001, TASK-INFRA-002, TASK-INFRA-003]
depends_on: [TASK-PET-001, TASK-PET-003]
blocks: [TASK-PET-006, TASK-PET-007, TASK-SOCIAL-003]
effort_hours: 8
new_files:
...
disallowed_tools:
  - "Real-money slot expansion (slots come bundled with Pet+ subscription per TASK-LEGAL-002 — no individual-slot IAP)"
  - "Slot count gating breeding (TASK-PET-007 has its own adult-stage gate)"
  - "Allowing the player to delete a tombstoned pet from their inventory (deletion is hard via DSR per TASK-LEGAL-001)"
risk_if_skipped: "Plan §PART 3 multi-pet system is the foundational P2 capability. Without slot inventory, TASK-PET-006 (5 species), TASK-PET-007 (breeding), TASK-SOCIAL-003 (trade) all break — they assume the player has multiple pet records they can manage."
audience_age_gate: "any"
---

## §1 — Description (BCP-14 normative)

§1.1  **Slot counts.** A free-tier player MUST be limited to **3** active pets. A Pet+ subscriber MUST be limited to **10** active pets. (TASK-SUB-001 implements the subscription; this task's `SlotEntitlementService` stubs the entitlement check until then — returning "free tier always" at P1/P2.)

§1.2  **Active vs total.** "Active" = `pets.status = 'active'`. Pets in `grandma_house` (Permadeath-Lite per TASK-PET-008) DO NOT count against the slot limit; the player can have unlimited grandma-house pets but only the slot count's worth of active ones.
...
§1.6  **Hatch enforcement.** `POST /v1/pets/hatch` (TASK-PET-001 §1.4) MUST check slot quota via this task's `InventoryService.canHatchNew(userId)`. On exceeded: HTTP 402 `{ error: 'pet_slot_full', current_count, slot_limit, suggest_upgrade: true }`.
...
§1.12  **Migration of legacy single-pet players.** Existing P1 players who have exactly 1 pet at the time TASK-PET-005 ships MUST be migrated to the new model:
...
§1.13  **Rate limit for hatch.** Per TASK-PET-001 §1.13, 3 hatches per 24h per player. This rule remains regardless of slot quota — even Pet+ players with empty slots can only hatch 3/day.
...
§1.15  **Cross-tenant scoping.** Inventory queries MUST respect `tenant_id` per TASK-INFRA-003 §1.4 — a Mochi (consumer) player's roster MUST NOT include B2B tenant pets, and vice versa. RLS policy `pets self read` (TASK-PET-001 §3.4) handles this when `app.tenant_id` is set.

§1.16  **Trade impact.** TASK-SOCIAL-003 (trade) MUST validate the receiving player has slot capacity BEFORE accepting a trade. The trade handler will call `canHatchNew(receiverId)` equivalent.

§1.17  **Breeding impact.** TASK-PET-007 (breeding) produces offspring pets. The parent's player MUST have slot capacity for the offspring; on hatching (post-24h incubation per TASK-PET-007), if slots are full, the egg is held in a "pending hatch" queue (24h grace period) before being lost.

§1.18  **Pet+ marketing surface — non-FOMO.** When a free-tier player tries to hatch beyond 3, the UX MUST: surface "Upgrade to Pet+ for 10 slots + premium AI + cloud save" in a non-shaming way (no urgency, no countdown). Plan §PART 6 + TASK-CARE-005 ethical retention principles.

§1.19  **Analytics.** `pet.slot.hatch_blocked`, `pet.slot.archive_to_grandma`, `pet.slot.upgrade_prompt_shown` per TASK-OBS-001.

§1.20  **DPO transparency.** `GET /v1/me/data` (a future DSR feature) MUST include slot history. Implementation slot here for TASK-LEGAL-001 §1.9 integration.
...
**Why no individual-slot IAP.** Plan §PART 6 + TASK-LEGAL-002 — randomised or atomic IAP undermines the subscription value proposition + creates regulator-questioned "pay to expand" surfaces. Slots come bundled with Pet+ only.
...
export class SlotEntitlementService {
  // P2 stub: always returns 'free'. TASK-SUB-001 (P3) wires real subscription state.
...
begin
  ...
  if active_count > slot_limit then
    raise exception 'TASK-PET-005 §1.11 slot quota exceeded for owner %', new.owner_id;
  end if;
  return new;
end;
$$ language plpgsql;
...
```typescript
// apps/api/src/pets/inventory/__tests__/inventory.spec.ts
describe('TASK-PET-005 — inventory', () => {
...
| 3 | Stub entitlement returns wrong tier | Player report | Slots wrong | TASK-SUB-001 lands; until then env var override |
...
| 8 | Offspring egg lost despite grace period | TASK-PET-007 audit | Player frustration | Cron job moves pending eggs |
...
**Cross-reference:** TASK-PET-006 (5 species), TASK-PET-007 (breeding offspring slot check), TASK-PET-008 (grandma_house archival), TASK-SOCIAL-003 (trade slot check), TASK-SUB-001 (entitlement).
