...
created: 2026-05-17
shipped: 2026-05-17
related_frs: [TASK-PET-001, TASK-PET-002, TASK-PET-004, TASK-INFRA-002, TASK-CARE-001, TASK-CARE-002, TASK-CARE-003, TASK-CARE-005, TASK-PET-008, TASK-AI-001, TASK-OBS-001]
depends_on: [TASK-PET-001, TASK-PET-002]
blocks: [TASK-CARE-001, TASK-CARE-002, TASK-CARE-003, TASK-CARE-005, TASK-PET-008]
effort_hours: 10
new_files:
...
disallowed_tools:
  - "Client-controlled stat values (server-authoritative only — anti-cheat per TASK-INFRA-002 §1.10)"
  - "Negative or > 100 stat values (clamp at boundaries)"
  - "Stats decaying during `grandma_house` stage (frozen until rescue)"
risk_if_skipped: "Stat-bar model is the core care loop signal — without it the daily session loop has nothing to render, TASK-CARE-001/002/003 have no state to mutate, TASK-PET-008 has no neglect signal, TASK-PET-002 has no care-gated stall input."
...
Rates configurable via Mixpanel feature flags (per TASK-OBS-001 §1.10). `egg` decays at 0 (pre-hatch — pet has no needs yet). `grandma_house` decays at 0 (pet is frozen in the rescue path).
...
Exact values configurable per TASK-OBS-001 feature flag. The cleanliness penalty on mini-game / sleep models real-world wear-and-tear.

§1.5  **Server-authoritative computation.** Every stat value MUST be computed server-side. The client never decides "your hunger is 75 now" — it asks the server, which holds authoritative state in Colyseus `PetState` + persists to Postgres on idle disposal (TASK-INFRA-002 §1.12).
...
§1.11  **Stat-bar rendering — accessibility.** The Cocos `StatBar.ts` component MUST: (a) render each bar with a colour-blind-safe palette (defaults to the TASK-A11Y-001 colour-blind preset when enabled); (b) show numeric value on long-press for screen-reader users; (c) animate transitions at ≤ 200 ms (capped at 50 ms under reduce-motion). Bar fill colours: green ≥ 50; yellow 30-49; orange 20-29; red < 20.

§1.12  **Persistence cadence.** `PetState` stat updates are persisted to `public.pets` table on: (a) every care action (immediate); (b) every 5 min if the room is active; (c) on `onDispose` (room idle 15 min, TASK-INFRA-002 §1.12). The Postgres columns MUST be added in the migration: `hunger int not null default 100, cleanliness int not null default 100, happiness int not null default 100, energy int not null default 100`.
...
§1.15  **Stat decay anti-cheat.** Direct client writes to `PetState.hunger` etc. MUST be impossible — Colyseus schema is server-only. Care actions go through server handlers per TASK-INFRA-002 §1.10. Impossible state transitions (e.g. negative stat values) emit `security.impossible-transition` Sentry event (TASK-INFRA-002 §1.9).
...
§1.17  **AI prompt context.** The current stats (and 24 h history) MUST be exposable to the AI personality engine (TASK-AI-001) as part of the persona-context payload. AI replies are aware of pet emotional state ("Mochi is feeling really hungry right now ...").

§1.18  **Co-parent visibility.** When TASK-SOCIAL-002 (co-parent) ships, the co-parent MUST see live stat-bars + care-action history. The schema MUST be co-parent-aware from day one (RLS extension is the path).

§1.19  **Feature-flag tunability.** All decay rates + care-action effects MUST be readable via Mixpanel feature flags. Defaults baked into code (TASK-OBS-001 §10 row 3 fallback).

§1.20  **Analytics + observability.** `pet.stat.changed { stat, prev, new, source }`, `pet.stat.warn`, `pet.stat.critical`, `pet.offline.reconciled` MUST be emitted per TASK-OBS-001 §1.4 taxonomy with schema validation. The mini-game / care-action source MUST be tagged so funnel analysis can identify which care actions drive retention.
...
**Why TimescaleDB hypertable for stat-history.** Stat history is high-write-volume time-series (one row per pet per minute). TimescaleDB compresses + auto-partitions this efficiently. 90-day retention with compression after 7 days gives TASK-PET-002 care-stall detection ample window without runaway storage. Same TimescaleDB pattern used in sale-noti project for price history — but as per the project-isolation rule, we don't cite that, we just adopt the proven pattern.

**Why broadcast warnings at 30 → 29 and 10 → 9.** These are the inflection points where a player should care + intervene. 30 is the TASK-PET-002 care-gated-stall threshold (warning gives the player a chance to act before the stall actually triggers). 10 is critical territory before TASK-PET-008 Permadeath-Lite kicks in.
...
**Why all care-action effects in one table.** Single source of truth simplifies live-ops tuning + task review. The Mixpanel feature-flag bindings consume the same table.
...
**Why co-parent visibility designed in day one.** Plan §PART 3 + TASK-SOCIAL-002 — PetPair is the viral wedge. If stat-bar visibility is owner-only at P1, P2 needs to retrofit; designing the RLS for co-parent at P1 is essentially free.
...
**AC11.** Co-parent visibility: a co-parent (TASK-SOCIAL-002 will activate) cannot read another player's stat-history at the schema level — verified by RLS test (even if TASK-SOCIAL-002 not yet shipped, the policy denies until the co-parent join row exists).
...
describe('TASK-PET-003 — stat decay', () => {
...
**Internal:** TASK-PET-001 (`pets` table + last_seen_at + cascade), TASK-PET-002 (`stage` column drives decay rate selection).

**Blocks:** TASK-CARE-001/002/003/004 (each care action calls `StatDecayService.apply`), TASK-CARE-005 (streak system reads stat history), TASK-PET-008 (Permadeath-Lite reads 7-day stat history).
...
**Cross-reference:** TASK-CARE-001/002/003/004/005 + TASK-PET-008 all depend on this model. TASK-AI-001 reads stat state for personality dialogue. TASK-PET-002 reads 24/72 h history for care-gated stall detection.
