...
created: 2026-05-17
shipped: 2026-05-17
related_frs: [TASK-ECON-002, TASK-I18N-001, TASK-LEGAL-001, TASK-LEGAL-002, TASK-AUTH-001, TASK-AUTH-002, TASK-OBS-001]
depends_on: [TASK-ECON-002, TASK-I18N-001]
blocks: []
effort_hours: 10
new_files:
...
allowed_tools:
  ...
  - "TASK-ECON-002 ledger atomic grant"
disallowed_tools:
  - "Direct PSP integration without MoR at P4 (compliance complexity)"
  - "Apple/Google subscriptions via Antom (must stay in-store per §1.6 TASK-SUB-001 alignment)"
...
§1.3  **One-off IAP only.** VN/PH/ID local payment for one-off IAP (currency packs, outfits, premium species). Subscriptions remain Apple/Google only per TASK-SUB-001 §`disallowed_tools`.
...
§1.5  **Pricing localisation.** Catalogue prices localised per TASK-ECON-002 §1.2 catalogue + TASK-I18N-001 currency formatting:
- VN: ₫29K-₫999K range.
- PH: ₱60-₱2,000 range.
...
§1.7  **Antom webhook.** Antom signs webhook with HMAC; receipt validation via Antom Server API. Per TASK-ECON-002 §1.7 server-side validation.

§1.8  **Xsolla webhook.** Same pattern; Xsolla-signed webhooks.

§1.9  **Refund handling.** Antom + Xsolla refund webhooks trigger entitlement revocation per TASK-ECON-002 §1.13.

§1.10  **Anti-fraud — Antom region match.** Per TASK-AUTH-001 §1.6 region-of-record — Antom payment must come from VN-region card/wallet. Mismatch flagged for review (potential fraud).
...
§1.12  **DPO data processing addendum.** Antom + Xsolla DPAs filed at `docs/legal/payment-mor-agreements.md`. Per TASK-LEGAL-001 §1.3 cross-border-TIA.
...
§1.20  **Analytics.** `iap.region.payment_initiated`, `iap.region.payment_completed`, `iap.region.payment_failed { reason }` per TASK-OBS-001.
...
**Why one-off IAP only via local rails.** TASK-SUB-001 — Apple/Google policy enforces subscription must be in-store.
...
```typescript
describe('TASK-I18N-002 — VN payment rails', () => {
...
**Cross-reference:** TASK-ECON-002 IAP base; TASK-AUTH-001 region; TASK-LEGAL-001 DPA.
