...
created: 2026-05-17
shipped: 2026-05-17
related_frs: [TASK-ECON-001, TASK-ECON-002, TASK-AUTH-001, TASK-PET-005, TASK-PET-007, TASK-AI-001, TASK-CARE-005, TASK-AR-001, TASK-SUB-002, TASK-INFRA-003, TASK-OBS-001]
depends_on: [TASK-ECON-002, TASK-AUTH-001]
blocks: [TASK-SUB-002]
effort_hours: 12
new_files:
...
disallowed_tools:
  ...
  - "Pet+ pricing >$4.99/mo without explicit task amendment"
...
§1.2  **Pet+ benefits.**
- 10 active pet slots (vs 3 free) — TASK-PET-005.
- 50% breeding cooldown reduction — TASK-PET-007.
- Premium AI persona quota raise (60 calls/pet/day vs 30 free) — TASK-AI-001.
- +1 forgiveness token monthly — TASK-CARE-005.
...
§1.5  **Entitlement service.** Replaces the stub at TASK-PET-005 §3.3. Returns `tier` based on `user_subscriptions.status` + `current_period_end`. Cached for 60 seconds.
...
§1.10  **Family Sharing.** Apple Family Sharing + Google Family Library entitlements honoured. Per TASK-LEGAL-003 §1.9.
...
§1.13  **Monthly Hearts grant.** On each renewal, the user is granted 100 Hearts via TASK-ECON-001 `iap_source` ledger entry. `ref = 'sub.monthly_grant:<period>'`.

§1.14  **Seasonal pet rotation.** Each month, Pet+ subscribers can claim a unique seasonal pet (e.g. "April Mochi", "May Pengu"). Per TASK-PET-006 — special-tier outside the 5 launch species (added to catalogue per-season).

§1.15  **Kids SKU + Family tier.** Pet+ on the kids SKU is bridged through TASK-SUB-002 Family tier — kids cannot independently subscribe. Per TASK-LEGAL-003.
...
§1.19  **Pricing change procedure.** Plan §PART 6 + Apple/Google rules — pricing changes require user opt-in for existing subscribers + 30-day notice. task amendment required.

§1.20  **Analytics.** `sub.subscribed`, `sub.renewed`, `sub.grace_started`, `sub.expired`, `sub.refunded`, `sub.restored`, `sub.cancelled` per TASK-OBS-001.
...
**Why no kids direct subscribe.** COPPA + TASK-LEGAL-001 — parental consent required.
...
| 7 | Pricing change without opt-in | Apple/Google reject | Submission blocked | task amendment |
...
**Cross-reference:** TASK-SUB-002 Family tier; TASK-PET-005/007 entitlement-consuming.
