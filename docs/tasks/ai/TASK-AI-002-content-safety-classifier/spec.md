...
created: 2026-05-17
shipped: 2026-05-17
related_frs: [TASK-AI-001, TASK-AI-003, TASK-PET-001, TASK-LEGAL-001, TASK-LEGAL-003, TASK-AUTH-003, TASK-VIRAL-002, TASK-VIRAL-003, TASK-OBS-001, TASK-SOCIAL-002]
depends_on: [TASK-AI-001, TASK-LEGAL-001]
...
disallowed_tools:
  ...
  - "Generative LLM dialogue on kids SKU (this task's scripted tree is the replacement)"
risk_if_skipped: "Without content safety, LLM hallucinations or jailbreak attempts ship to players — direct Apple/Google App Review rejection risk + COPPA-2025 enforcement risk if kids-adjacent. TASK-PET-001 naming filter and TASK-AI-001 persona output both depend on this classifier."
audience_age_gate: "any"
---

## §1 — Description (BCP-14 normative)

§1.1  **Universal output gate.** Every LLM-generated text (TASK-AI-001 personas, TASK-VIRAL-002 Daily Drama, TASK-VIRAL-003 generative pet, TASK-AI-003 personality v2) MUST pass through this service before reaching the client.
...
§1.5  **Kids-SKU stricter rules.** Per TASK-LEGAL-001 §1.5(b) + TASK-AUTH-003, the kids SKU MUST use ONLY the scripted dialogue tree — generative LLM is forbidden. The scripted tree consists of pre-authored phrases stored in `scripts/<locale>/<stage>.json`.
...
§1.8  **Kids endpoint for scripted dialogue.** `POST /v1/ai/pet/:petId/scripted-speak` — body `{ trigger: ScriptedTrigger }`. Returns scripted phrase from the appropriate `scripts/<locale>/<stage>.json` randomly chosen (TASK-LEGAL-002 deterministic-by-seed isn't required because scripted dialogue isn't a randomised outcome that affects monetised inventory).
...
§1.12  **Block-without-substitute.** If `ok: false`, the calling service (TASK-AI-001 etc.) is responsible for substituting a scripted fallback. This task does NOT provide a substitute — that's the caller's job.
...
§1.17  **Scripted dialogue localisation.** All scripts MUST be authored in EN + VI for P1. Other locales follow in P4 via TASK-I18N-001.
...
§1.19  **Cocos integration.** `AiSpeechBubble.ts` (TASK-AI-001 §1.18) handles both `claude-haiku`/`gemini-flash` source-model AND `scripted_fallback`/`scripted_kids` identically — same render path.
...
**Why two vendors.** Single-vendor failure = no safety = halt to LLM features. Two-vendor redundancy matches the TASK-AI-001 model architecture.
...
**Why blocked-without-substitute is the caller's responsibility.** Separation of concerns — this task classifies; callers decide fallback strategy (TASK-AI-001 → scripted; TASK-VIRAL-002 → generic phrase; etc.).
...
```typescript
// apps/api/src/ai/safety/__tests__/content-safety.spec.ts
describe('TASK-AI-002 — content safety', () => {
...
**Cross-reference:** Every LLM output task depends on this. The scripted tree replaces generative chat in the kids SKU.
