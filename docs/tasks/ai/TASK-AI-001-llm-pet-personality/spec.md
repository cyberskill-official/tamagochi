...
created: 2026-05-17
shipped: 2026-05-17
related_frs: [TASK-PET-001, TASK-PET-002, TASK-PET-003, TASK-AI-002, TASK-INFRA-002, TASK-INFRA-003, TASK-CARE-001, TASK-CARE-002, TASK-CARE-003, TASK-VIRAL-002, TASK-VIRAL-003, TASK-AI-003, TASK-OBS-001, TASK-LEGAL-001]
depends_on: [TASK-PET-002, TASK-INFRA-002]
blocks: [TASK-AI-002, TASK-VIRAL-002, TASK-VIRAL-003, TASK-AI-003]
effort_hours: 12
new_files:
...
disallowed_tools:
  - "Generative LLM chat to under-13 SKU (TASK-AI-002 wires the scripted-tree replacement)"
  - "Free-form user-supplied prompts to the LLM (system prompt + structured event context only)"
  - "Storing raw LLM responses without content-safety filter (TASK-AI-002 gate)"
...
§1.1  **Standard SKU only.** Per TASK-LEGAL-001 §1.5 + TASK-AUTH-003, LLM generative chat is forbidden in the kids SKU. Under-13 pets use scripted dialogue trees (TASK-AI-002). This task specs the 13+ SKU pipeline only.
...
§1.9  **Cost cap per pet per day.** Each pet has a daily LLM-call cap: 30 calls. Beyond, the persona falls back to a 12-line scripted bank (TASK-AI-002 reuses these for the kids SKU). Cap reset at player's local midnight.
...
§1.11  **Content safety gating.** Every LLM response MUST pass through TASK-AI-002's content-safety classifier BEFORE being returned to the client. Failures emit `ai.persona.safety_blocked` event + a generic safe response substitutes.
...
```typescript
{
  ...
  animation_hint: 'idle_baby'|'happy'|'dance'|'sad'|...;  // TASK-ART-001 contract name
...
§1.17  **Sentiment → animation mapping.** The LLM response includes a sentiment field; server maps it to a TASK-ART-001 contract animation: happy → `happy`, sad → `sad`, tired → `sick`, excited → `dance`, neutral → idle. The mapping table is `apps/api/src/ai/persona/sentiment-anim-map.ts`.
...
§1.20  **Cost monitoring + alerts.** Daily aggregated LLM spend per tenant tracked. Alert when daily spend exceeds $50 (initial budget) — surface in OBS dashboard. Per TASK-OBS-001 critical-incident rules.
...
**Why scripted fallback when cap hit.** Plan §PART 4 — keep cost predictable. Falling back to scripted phrases at cap is invisible to the player as long as the bank is varied (TASK-AI-002's 12 lines is the minimum).
...
**Why content-safety gating on output.** LLM hallucinations + jailbreak attempts can produce harmful text. TASK-AI-002 classifier is the canonical gate.
...
**Why kids SKU explicitly excluded.** Snap My AI / Replika regulatory precedent. Plan §PART 4. Scripted trees in TASK-AI-002.
...
```json
{ "error": "ai.kids.forbidden", "guidance": "Use scripted dialogue tree via TASK-AI-002." }
```
...
| 5 | Content-safety classifier flags too aggressively | Scripted ratio spikes | UX dull | Tune TASK-AI-002 thresholds |
...
| 11 | Kid mistakenly authenticated on standard SKU → AI exposure | RLS via sub claim | 403 returned | TASK-AUTH-003 / SKU gate enforces |
| 12 | Sentiment-to-animation map drifts from contract names | Boot-time check | Server refuses to start | Locked contract names from TASK-ART-001 |
...
**Cross-reference:** TASK-AI-002 is the safety classifier this task depends on. TASK-VIRAL-002 (Daily Drama), TASK-VIRAL-003 (generative pet), TASK-AI-003 (personality v2) build on top.
