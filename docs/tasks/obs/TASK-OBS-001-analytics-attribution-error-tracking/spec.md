...
created: 2026-05-17
shipped: 2026-05-17
related_frs: [TASK-INFRA-001, TASK-INFRA-002, TASK-INFRA-003, TASK-LEGAL-001, TASK-LEGAL-002, TASK-LEGAL-003, TASK-ADS-002, TASK-OBS-002, TASK-B2B-002]
depends_on: [TASK-INFRA-001, TASK-INFRA-003, TASK-LEGAL-001]
blocks: [TASK-PET-001, TASK-CARE-001, TASK-OBS-002]
effort_hours: 8
new_files:
...
disallowed_tools:
  - "ANY behavioural analytics SDK in the kids SKU (Mixpanel / Amplitude / AppsFlyer / Adjust / AppLovin / IronSource all banned per TASK-LEGAL-001 §1.5(a))"
...
§1.2  **Kids-SKU SDK stack — radically reduced.** The kids SKU MUST integrate ONLY: (a) **GameAnalytics** with `consent_mode: "kids_safe"` (no behavioural-ID tracking); (b) **Sentry** for crash reporting (Sentry's `sendDefaultPii: false` mode). No Mixpanel / Amplitude / AppsFlyer / Adjust / Firebase. Verified at binary level by TASK-LEGAL-003 §1.10 inspection script.

§1.3  **SDK allow-list config.** `apps/cocos/src/__sdk-allowlist/kids.json` and `apps/cocos/src/__sdk-allowlist/standard.json` MUST enumerate the SDK packages and their version pins. The build script (TASK-INFRA-001 §1.4) MUST consume the SKU's allow-list and reject the build if any imported package is NOT on the allow-list for that SKU.

§1.4  **Event taxonomy.** A canonical event taxonomy MUST live at `docs/obs/event-taxonomy.md` covering: `auth.*`, `pet.*`, `care.*`, `ai.*`, `ar.*`, `viral.*`, `social.*`, `econ.*`, `sub.*`, `ad.*`, `legal.*`, `security.*`, `infra.*`. Every event name MUST follow `<module>.<action>.<outcome>` snake-case format. New event names require an task amendment.
...
§1.8  **Sentry release tagging.** Every release MUST tag Sentry with the `git rev` + a human-readable version (e.g. `tamagochi@1.0.3-rc.2+abc1234`). Source maps for the web build MUST be uploaded to Sentry on every CI deploy (per TASK-INFRA-001 §1.17).

§1.9  **Sentry transaction sampling.** Backend transactions sample at 5% by default; auth-flow transactions sample at 50% (lower volume); error transactions sample at 100%. Per-tenant tail sampling (B2B in P4) is deferred to TASK-OBS-002.
...
§1.12  **GameAnalytics economy events.** GameAnalytics MUST receive `BusinessEvent`, `ResourceEvent`, `ProgressionEvent`, `DesignEvent` per its documented model. The economy taxonomy MUST cover all currency sources + sinks defined in TASK-ECON-001. The kids SKU sends only `DesignEvent` (no business — no IAP in kids SKU directly).
...
**AC2.** Kids SKU build links ONLY GameAnalytics + Sentry. Verified by TASK-LEGAL-003 §1.10 binary inspection — `strings <kids.ipa> | grep -iE "(Mixpanel|AppsFlyer|Adjust|AppLovin|IronSource|Firebase)"` returns empty.
...
describe('TASK-OBS-001 §1.3 — kids SDK allow-list', () => {
...
**Internal:** TASK-INFRA-001 (Cocos scaffold); TASK-INFRA-003 (Supabase for event audit); TASK-LEGAL-001 (PII scrub rules); TASK-LEGAL-003 (binary inspection for SDK allow-list); TASK-AUTH-001..003 (the `identify` call hooks into auth state).

**Blocks:** TASK-PET-001 (will emit `pet.hatched`); TASK-CARE-001..005 (care events); TASK-OBS-002 (anti-cheat builds on this metrics scaffold).
...
- **OQ-4 (resolved):** Tail sampling per tenant? → §1.9 + deferred to TASK-OBS-002 in P4.
...
|---|---|---|---|---|
| 1 | A denied SDK leaks into kids binary | TASK-LEGAL-003 §1.10 + `__tests__/kids-sdk-allowlist.spec.ts` | Build blocked | Identify transitive dep; replace or vendor-isolate |
...
**Cross-reference:** TASK-OBS-001 is the observability substrate. TASK-OBS-002 (P4 anti-cheat audit) builds on top. Every gameplay task (TASK-PET-001 onward) emits events into this scaffold.
