...
const OUT_DIR = join(ROOT, 'docs/feature-requests/pipeline');
...
async function frFiles() {
  ...
  for (const folder of await fs.readdir(FR_ROOT, { withFileTypes: true })) {
    ...
    for (const file of await fs.readdir(join(FR_ROOT, folder.name))) {
      if (/^FR-.*\.md$/.test(file) && !file.endsWith('.audit.md')) out.push(join(FR_ROOT, folder.name, file));
    }
  }
  return out;
}
...
async function assess(fr, completed = new Set()) {
  const missing = [];
  const scaffold = [];
...
async function writeManifest(records, label) {
  ...
  const lines = [
    '# Tamagochi Strict FR Task Manifest',
    '',
    ...
    '## FR States',
    '',
    '| # | FR-ID | Phase | Module | State | Reason |',
    '|---:|---|---|---|---|---|',
...
async function main() {
  ...
  for (const id of BUILD_ORDER) {
    ...
    if (!fr) throw new Error(`Missing ${id}`);
    const row = { ...fr, ...(await assess(fr, completed)), attempts: 0 };
    records.push(row);
    ...
    const checks = [
      ...
      await run('npm', ['run', 'test:fr', '--', '--test-name-pattern', row.id]),
      await run('npm', ['run', 'test:e2e']),
      await run('npm', ['run', 'fr:check']),
